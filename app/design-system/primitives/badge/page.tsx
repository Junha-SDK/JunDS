...
const allVariants: BadgeVariant[] = ["default", "primary", "success", "warning", "danger", "info", "outline"];
...
export default function BadgePage() {
  return (
    <ComponentPage
      ...
      version="1.0.0"
      props={[
        { name: "variant", type: '"default" | "primary" | "success" | "warning" | "danger" | "info" | "outline"', default: '"default"', description: "Badge의 색상 스타일을 결정합니다. 전달하려는 정보의 성격에 맞는 변형을 선택하세요." },
        { name: "size", type: '"sm" | "md" | "lg"', default: '"md"', description: "Badge의 크기를 설정합니다." },
        { name: "dot", type: "boolean", default: "false", description: "라벨 앞에 상태를 나타내는 컬러 점을 표시합니다." },
        { name: "count", type: "number", description: "숫자 카운트 모드로 전환합니다. 알림 개수 등에 사용합니다." },
        { name: "maxCount", type: "number", default: "99", description: "count 모드에서 표시할 최대 숫자입니다. 초과 시 '99+' 형태로 표시됩니다." },
        { name: "children", type: "ReactNode", description: "Badge 내부에 표시할 텍스트 또는 요소" },
      ]}
      related={[
...
<Badge size="lg" variant="primary">Large</Badge>`}
        ...
        <UsageNote type="info">
          <code className="text-xs font-mono bg-gray-100 px-1 rounded">sm</code>은 테이블 셀, 목록 항목 등 공간이 제한된 곳에 적합합니다. <code className="text-xs font-mono bg-gray-100 px-1 rounded">lg</code>는 페이지 제목 옆이나 강조가 필요한 곳에 사용하세요.
        </UsageNote>
      </Section>
...
<Badge count={999} maxCount={999} />`}
        ...
        <UsageNote type="tip">
          <code className="text-xs font-mono bg-gray-100 px-1 rounded">maxCount</code>를 설정하면 큰 숫자를 간결하게 표현할 수 있습니다. 기본값은 99이며, 초과 시 &quot;99+&quot;로 표시됩니다.
        </UsageNote>
      </Section>

      {/* ── 실제 활용 예시 ── */}
      <Section title="실제 활용 예시" description="Badge가 다양한 UI 맥락에서 어떻게 활용되는지 보여줍니다.">
        <Preview>
          <div className="flex flex-col gap-4 w-full max-w-md">
            ...
            <div className="flex items-center justify-between px-4 py-3 rounded-lg border border-border-light">
              ...
              <Badge variant="success" dot size="sm">정상</Badge>
            </div>
            {/* 카테고리 태그 */}
      ...
      <Section title="가이드라인">
        <Guidelines
          items={[
            {
              type: "do",
              description: "짧고 명확한 텍스트만 사용하세요. 한두 단어가 이상적입니다. (예: '신규', '완료', 'Beta')",
            },
            {
              type: "dont",
              description: "문장이나 긴 설명을 Badge에 넣지 마세요. Badge는 라벨이지, 문단이 아닙니다.",
            },
            {
              type: "do",
              description: "의미에 맞는 색상 변형을 일관되게 사용하세요. success는 긍정적 상태, danger는 부정적 상태에 사용합니다.",
            },
            {
              type: "dont",
              description: "하나의 요소에 3개 이상의 Badge를 나열하지 마세요. 정보 과부하로 가독성이 떨어집니다.",
            },
            {
              type: "caution",
              description: "색상만으로 정보를 전달하지 마세요. 색각 이상이 있는 사용자를 위해 텍스트와 함께 사용합니다.",
            },
            {
              type: "do",
              description: "카운트 배지는 실시간 업데이트가 필요한 곳(알림, 메시지 등)에 사용하세요.",
            },
          ]}
        />
      </Section>
