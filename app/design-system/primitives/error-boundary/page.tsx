...
function DemoDefault() {
  ...
  return (
    <div className="flex flex-col gap-3 max-w-md">
      ...
      <Button size="sm" variant="danger" onClick={() => setBoom(true)} disabled={boom}>
        오류 발생시키기
      </Button>
      {boom && (
        <Button size="sm" variant="ghost" onClick={() => setBoom(false)}>
          리셋
        </Button>
      )}
    </div>
  ...
  return (
    <ComponentPage
      ...
      props={[
        { name: "children", type: "ReactNode", description: "오류를 감시할 자식 트리." },
        {
          ...
          description: "오류 발생 시 표시할 UI. 함수면 Error 객체와 reset 콜백을 인자로 받습니다.",
        },
        {
          name: "onError",
          type: "(error: Error, errorInfo: ErrorInfo) => void",
          description: "오류 발생 시 호출되는 콜백 (로깅용).",
        },
    ...
    >
      <Section
        ...
        description="fallback을 지정하지 않으면 기본 오류 UI가 표시됩니다."
      >
        <Preview>
          <DemoDefault />
        </Preview>
      ...
      <Section
        ...
        description="함수형 fallback으로 error와 reset을 받아 직접 UI를 구성합니다."
      >
        <Preview>
          <DemoCustom />
        </Preview>
