...
const navItems = [
  ...
  {
    icon: "M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12",
    label: "분석",
  },
  {
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left">
                    <th className="px-4 py-2.5 text-xs font-semibold text-muted uppercase">
                      주문번호
                    </th>
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr
                      key={order.id}
                      className="border-b border-border-light last:border-0 hover:bg-gray-50/50"
                    >
                      <td className="px-4 py-3 font-mono text-xs">{order.id}</td>
                      <td className="px-4 py-3 flex items-center gap-2">
                        <Avatar name={order.customer} size="xs" />
                        {order.customer}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <Badge
                          variant={statusMap[order.status as keyof typeof statusMap].variant}
                          size="sm"
                          dot
                        >
