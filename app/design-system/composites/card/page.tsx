...
export default function CardPage() {
  return (
      ...
      props={[
        {
          ...
          default: "false",
          description:
            "마우스 호버 시 그림자 강화 및 약간의 상승 효과를 적용합니다. 클릭 가능한 카드에 사용하세요.",
        },
        {
          name: "noPadding",
          type: "boolean",
          default: "false",
          description: "카드 내부 패딩을 제거합니다. 이미지나 커스텀 레이아웃에 유용합니다.",
        },
        {
          name: "asChild",
          type: "boolean",
          default: "false",
          description:
            "Radix-style Slot. true이면 자체 div 대신 단일 자식 엘리먼트의 root에 className/style/이벤트를 합칩니다. <Link>, <button> 등으로 위임할 때 사용합니다.",
        },
        { name: "className", type: "string", description: "카드 컨테이너에 추가할 커스텀 클래스" },
        {
          ...
          type: "ReactNode",
          required: true,
          description: "Card.Header, Card.Body, Card.Footer 등 서브 컴포넌트를 포함합니다.",
        },
      ]}
      related={[
    ...
    >
      ...
      <Section
        title="개요"
        description="Card는 Header, Body, Footer를 조합하여 콘텐츠를 체계적으로 구성합니다."
      >
        <Preview
          sourceCode={`<Card>
  ...
  <Card.Footer>
    <div className="flex items-center justify-between">
            ...
            <Card>
              ...
              <Card.Body>
                <p className="text-sm text-muted">
                  현재 진행 중인 프로젝트의 전체 상태를 한눈에 확인할 수 있습니다. 세부 사항은
                  대시보드에서 확인하세요.
                </p>
              </Card.Body>
              <Card.Footer>
      ...
      <Section title="구조 (Anatomy)">
        <Anatomy
          items={[
            {
              label: "Container",
              description: "카드의 최외곽 래퍼입니다. 테두리, 둥근 모서리, 그림자를 제공합니다.",
            },
            {
              label: "Header",
              description:
                "카드 상단의 제목 영역입니다. 문자열을 전달하면 자동으로 h3 태그로 렌더링됩니다.",
            },
            {
              label: "Body",
              description:
                "카드의 메인 콘텐츠 영역입니다. 텍스트, 이미지, 리스트 등 자유롭게 배치할 수 있습니다.",
            },
            {
              label: "Footer",
              description:
                "카드 하단의 액션 영역입니다. 버튼, 링크 등 사용자 인터랙션 요소를 배치합니다.",
            },
          ]}
        />
      </Section>
      ...
      <Section
        title="변형 (Variants)"
        description="Card는 다양한 스타일과 조합으로 활용할 수 있습니다. 각 변형의 우상단 '코드' 버튼으로 즉시 복사하세요."
      >
        <VariantGrid cols={2}>
          <VariantItem
            label="기본 (Default)"
  ...
  <Card.Footer>
    <Button size="sm" variant="secondary">영수증 보기</Button>
      ...
      {/* ── 의사결정 매트릭스 ── */}
      <Section
        title="비슷한 카드 컴포넌트 비교"
        description="Card 외에도 카드 형태의 전용 컴포넌트가 있습니다. 의도에 맞춰 선택하세요."
      >
        <DecisionMatrix
          rows={[
            {
              ...
              signature: "범용 컨테이너",
              useWhen:
                "Header / Body / Footer 조합으로 자유롭게 콘텐츠를 그룹화할 때. 가장 무난한 기본 선택.",
              avoidWhen:
                "지표·강조·리스트 항목처럼 의미가 명확한 경우 — 전용 컴포넌트가 더 명확한 의도를 전달.",
            },
            {
              name: "StatCard",
              ...
              signature: "지표 표시",
              useWhen:
                "단일 지표(숫자 + 라벨 + 변화율)를 카드 한 장에 보여줄 때. 대시보드 상단 KPI에 적합.",
              avoidWhen:
                "여러 메트릭이나 비주얼이 함께 들어가야 한다면 MetricCard나 ChartCard를 사용.",
            },
            {
              name: "MetricCard",
              ...
              signature: "지표 + 미니차트",
              useWhen:
                "지표 옆에 스파크라인 등 추세 시각화를 함께 보여줄 때. 분석 대시보드의 카드 그리드.",
            ...
            },
            {
              ...
              signature: "책/제품",
              useWhen:
                "표지 이미지 + 제목 + 저자 같은 책 메타데이터 패턴 — 도서·강의·콘텐츠 카탈로그.",
            ...
            <Card>
              <Card.Body>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary-ink font-bold text-sm">
                    JH
                  </div>
                  <div>
                ...
                <div className="flex gap-4 text-xs text-muted">
                  <span>
                    <strong className="text-foreground">128</strong> 커밋
                  </span>
                  <span>
                    <strong className="text-foreground">34</strong> PR
                  </span>
                  <span>
                    <strong className="text-foreground">12</strong> 리뷰
                  </span>
                </div>
              </Card.Body>
            </Card>
      ...
      <Section
        title="noPadding 활용"
        description="이미지나 커스텀 레이아웃을 사용할 때 noPadding 옵션으로 내부 여백을 제거할 수 있습니다."
      >
        <Preview>
        ...
        <UsageNote type="info">
          <code className="text-xs font-mono bg-gray-100 px-1 rounded">noPadding</code>을 사용할
          때는 서브 컴포넌트(Header, Body, Footer) 대신 직접 레이아웃을 구성하는 것이 더 유연합니다.
        </UsageNote>
      </Section>

      {/* ── asChild — Slot 위임 ── */}
      <Section
        title="asChild — 외부 컨테이너 위임"
        description="asChild={true}이면 Card는 자체 <div>를 렌더하지 않고 단일 자식 엘리먼트로 위임합니다. Next의 <Link>, <button> 등 임의의 컨테이너로 카드 디자인을 적용할 수 있습니다."
      >
        <Preview>
          <div className="max-w-sm w-full">
        ...
        <UsageNote type="tip">
          asChild를 쓸 때 자식은 <strong>정확히 하나</strong>의 React element여야 합니다.
          문자열·fragment·복수 자식은 dev 환경에서 console.warn 후 null이 렌더됩니다. className은
          tailwind-merge로 병합되어 자식 className이 충돌 시 우선합니다.
        </UsageNote>
        <CodeExample
          ...
          items={[
            {
              type: "do",
              description:
                "관련된 정보를 하나의 카드로 그룹화하세요. 카드는 독립적인 콘텐츠 단위로 동작해야 합니다.",
            },
            {
              type: "dont",
              description:
                "카드 안에 카드를 중첩하지 마세요. 시각적 계층 구조가 혼란스러워지고 레이아웃이 복잡해집니다.",
            },
            {
              type: "do",
              description:
                "같은 그리드 내의 카드는 동일한 구조(Header/Body/Footer 조합)를 유지하여 시각적 일관성을 확보하세요.",
            },
            {
              type: "dont",
              description:
                "카드 내부에 과도한 양의 콘텐츠를 넣지 마세요. 핵심 정보만 요약하고, 상세 내용은 별도 페이지로 이동하세요.",
            },
            {
              type: "caution",
              description:
                "hoverable 카드는 반드시 클릭 가능한 인터랙션이 있어야 합니다. 시각 효과만 있고 동작이 없으면 사용자를 혼란스럽게 합니다.",
            },
            {
              type: "do",
              description:
                "Footer에는 주요 액션 버튼을 1~2개로 제한하세요. 너무 많은 버튼은 선택 부담을 증가시킵니다.",
            },
          ]}
        />
      </Section>
