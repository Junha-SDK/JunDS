...
export default function ModalPage() {
  ...
  return (
    <ComponentPage
      ...
      props={[
        ...
        },
        {
          ...
          required: true,
          description:
            "모달이 닫혀야 할 때 호출되는 콜백입니다. ESC 키, 배경 클릭, 닫기 버튼 등에서 트리거됩니다.",
        },
        {
          name: "size",
          type: '"sm" | "md" | "lg" | "xl" | "full"',
          default: '"md"',
          description: "모달 너비를 결정합니다. 컨텐츠 양에 맞게 선택하세요.",
        },
        {
          name: "dismissible",
          type: "boolean",
          default: "true",
          description:
            "배경(backdrop) 클릭으로 닫기를 허용할지 결정합니다. 중요한 확인 모달에서는 false로 설정하세요.",
        },
        {
          name: "className",
          type: "string",
          description: "모달 컨테이너에 추가할 커스텀 클래스입니다.",
        },
      ]}
      related={[
    ...
    >
      ...
      <Section
        title="Overview"
        description="모달은 현재 페이지 위에 떠오르는 대화 상자입니다. 사용자가 중요한 결정을 내리거나, 추가 정보를 입력하거나, 경고를 확인해야 할 때 사용합니다. 아래 버튼을 클릭하여 직접 체험해 보세요."
      >
        <Preview
...
<Modal open={open} onClose={() => setOpen(false)} size="md">
  ...
  <Modal.Footer>
    ...
    <Button onClick={() => setOpen(false)}>확인</Button>
          ...
          <Modal open={open} onClose={() => setOpen(false)} size={size}>
            ...
            <div className="p-5">
              <p className="text-sm text-muted leading-relaxed">
                이 모달은 <strong>{size}</strong> 크기입니다. ESC 키를 누르거나 배경을 클릭하여 닫을
                수 있습니다. 모달이 열리면 포커스가 자동으로 모달 내부로 이동하며, Tab 키로
                순환합니다.
              </p>
            </div>
            <Modal.Footer>
      ...
      <Section
        title="Anatomy"
        description="Modal 컴포넌트의 구조를 이해하면 올바르게 조합할 수 있습니다."
      >
        <Anatomy
          items={[
            {
              label: "Backdrop",
              description:
                "모달 뒤에 깔리는 반투명 배경입니다. 클릭 시 onClose가 호출됩니다 (dismissible=true일 때).",
            },
            {
              label: "Container",
              description:
                "모달 콘텐츠를 감싸는 흰색 카드입니다. size prop에 따라 max-width가 달라집니다.",
            },
            {
              label: "Header (Modal.Header)",
              description:
                "모달 상단의 제목과 닫기 버튼 영역입니다. onClose를 전달하면 X 버튼이 자동으로 표시됩니다.",
            },
            {
              label: "Body",
              description:
                "모달의 주요 콘텐츠 영역입니다. 별도 서브 컴포넌트 없이 자유롭게 구성합니다.",
            },
            {
              label: "Footer (Modal.Footer)",
              description:
                "모달 하단의 액션 버튼 영역입니다. 취소/확인 버튼을 배치합니다. 우측 정렬이 기본입니다.",
            },
          ]}
        />
      </Section>
      ...
      <Section
        title="Sizes"
        description="콘텐츠의 양과 복잡도에 따라 5가지 크기를 선택할 수 있습니다."
      >
        <VariantGrid cols={3}>
          <VariantItem
            label="Small (sm)"
      ...
      {/* ── Decision Matrix ── */}
      <Section
        title="비슷한 오버레이 컴포넌트 비교"
        description="모달 외에도 화면 위에 떠오르는 컴포넌트가 다양합니다. 의도와 화면 비율에 맞는 것을 선택하세요."
      >
        <DecisionMatrix
          rows={[
            {
              ...
              signature: "중앙 다이얼로그",
              useWhen:
                "확인·폼·상세 정보 등 사용자의 흐름을 잠시 멈추고 집중시키는 일반 다이얼로그.",
            ...
            },
            {
              ...
              signature: "측면 슬라이드",
              useWhen:
                "필터·상세 패널처럼 메인 화면의 맥락을 유지한 채 좌/우에서 슬라이드되는 보조 화면.",
      ...
      <Section
        title="주요 기능"
        description="Modal은 접근성과 UX를 위한 핵심 기능을 기본 내장하고 있습니다."
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-xl border border-border p-4">
            ...
            <p className="text-xs text-muted leading-relaxed">
              모달이 열리면 포커스가 내부에 갇힙니다. Tab 키로 순환하며 마지막 요소에서 첫 번째로
              돌아갑니다.
            </p>
          </div>
          <div className="rounded-xl border border-border p-4">
            ...
            <p className="text-xs text-muted leading-relaxed">
              키보드의 Escape 키를 누르면 즉시 모달이 닫힙니다. 별도 설정 없이 자동 동작합니다.
            </p>
          </div>
          <div className="rounded-xl border border-border p-4">
            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center mb-3">
            ...
            <p className="text-xs text-muted leading-relaxed">
              반투명 배경을 클릭하면 모달이 닫힙니다. dismissible=false로 비활성화할 수 있습니다.
            </p>
          </div>
        </div>
      </Section>
          ...
          items={[
            {
              type: "do",
              description:
                "중요한 결정이나 확인이 필요한 상황에서만 모달을 사용하세요. 삭제 확인, 결제 승인, 중요 설정 변경 등이 적합합니다.",
            },
            {
              type: "dont",
              description:
                "모달 안에 또 다른 모달을 중첩하지 마세요. 사용자가 맥락을 잃고 혼란스러워집니다. 복잡한 흐름은 페이지 전환으로 처리하세요.",
            },
            {
              type: "do",
              description:
                "모달의 Footer에는 명확한 액션 버튼을 배치하세요. 주요 액션(확인)은 우측에, 보조 액션(취소)은 좌측에 놓습니다.",
            },
            {
              type: "caution",
              description:
                "긴 스크롤이 필요한 콘텐츠는 모달 대신 새 페이지나 Drawer를 고려하세요. 모달 내부 스크롤은 사용성을 저하시킵니다.",
            },
          ]}
        />
      </Section>
      ...
      <Section title="사용 시 참고사항">
        <div className="flex flex-col gap-3">
          <UsageNote type="info">
            Modal은 React Portal을 사용하여 DOM 트리 최상위에 렌더링됩니다. 부모 요소의{" "}
            <code className="text-xs bg-gray-100 px-1 rounded">overflow: hidden</code>이나
            <code className="text-xs bg-gray-100 px-1 rounded">z-index</code>에 영향을 받지
            않습니다.
          </UsageNote>
          <UsageNote type="tip">
            모달이 열릴 때 <code className="text-xs bg-gray-100 px-1 rounded">body</code>의 스크롤이
            자동으로 잠깁니다. 닫히면 원래 상태로 복원됩니다.
          </UsageNote>
          <UsageNote type="warning">
            <code className="text-xs bg-gray-100 px-1 rounded">dismissible=false</code>로 설정하면
            반드시 모달 내부에 명시적인 닫기 수단(버튼)을 제공하세요. 그렇지 않으면 사용자가 모달을
            벗어날 수 없습니다.
          </UsageNote>
        </div>
      </Section>
      ...
      <Section
        title="코드 예시"
        description="Modal을 Header, Body, Footer로 구성하는 기본 패턴입니다."
      >
        <CodeExample
