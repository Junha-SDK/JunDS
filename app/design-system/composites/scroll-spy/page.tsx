...
export default function ScrollSpyPage() {
  return (
    <ComponentPage
      ...
      props={[
        { name: "sections", type: "ScrollSpySection[]", required: true, description: "섹션 목록 ({ key, label, targetId })" },
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-8">
              {sections.map((section) => (
                <div key={section.key} id={section.targetId} className="border border-border rounded-lg p-6">
                  <h3 className="text-base font-semibold mb-2">{section.label}</h3>
                  <p className="text-sm text-muted">
                    이 섹션은 &quot;{section.label}&quot; 영역입니다. 페이지를 스크롤하면 좌측 네비게이션에서
                    현재 보이는 섹션이 하이라이트됩니다.
                  </p>
                </div>
              ))}
