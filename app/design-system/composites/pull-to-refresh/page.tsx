...
export default function PullToRefreshPage() {
  ...
  return (
    <ComponentPage
      ...
      props={[
        { name: "onRefresh", type: "() => Promise<void>", description: "당김 임계값을 넘었을 때 실행할 비동기 함수" },
        { name: "threshold", type: "number", default: "60", description: "새로고침 트리거 거리(px)" },
      ...
      <Section title="Default">
        <Preview>
          <div className="w-full max-w-sm">
            <PullToRefresh onRefresh={refresh} className="h-72 rounded-xl border border-border bg-white">
              <ul className="divide-y divide-border">
                {items.map((it) => (
                  <li key={it} className="px-4 py-3 text-sm">{it}</li>
                ))}
              </ul>
            </PullToRefresh>
