  ...
  return (
    <>
      ...
      <button
        ...
        className={cn(
          "fixed bottom-6 right-6 z-50 w-12 h-12 rounded-full flex items-center justify-center cursor-pointer",
          // 떠 있는 버튼에 shadow-lg 한 겹은 배경에 붙어 보인다 — 넓은 그림자 +
          // 가까운 그림자 + 얇은 링을 겹쳐 페이지 위로 들어 올린다
          "shadow-[0_12px_28px_-8px_rgba(0,0,0,0.38),0_4px_10px_-4px_rgba(0,0,0,0.24)]",
          "ring-1 ring-black/5",
          // 바뀌는 것은 색과 변형뿐이다 — all 은 width·radius 까지 전이 대상으로 삼는다
      ...
      {open && (
        <div
          className={cn(
            "fixed bottom-20 right-6 z-50 w-80 bg-card border border-border rounded-2xl",
            // 떠 있는 패널 — shadow-2xl 한 겹은 유령이다. 다층 그림자 + 얇은 링
            "shadow-[0_24px_56px_-16px_rgba(0,0,0,0.45),0_8px_18px_-8px_rgba(0,0,0,0.28)]",
            "ring-1 ring-black/5",
        ...
        >
          ...
          <div className="px-4 py-3 border-b border-border bg-surface-soft">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className={cn(
                  "shrink-0 p-1 rounded-lg text-muted hover:text-foreground hover:bg-border-light",
          ...
          <div className="p-4 space-y-5 max-h-[60vh] overflow-y-auto">
            {/* Theme */}
            <div>
              <div className="block text-[10px] font-bold text-muted uppercase tracking-widest mb-2">
                테마 색상
              </div>
              <div className="flex flex-wrap gap-2">
                {THEMES.map((t) => (
                  <button
                    ...
                    // 색만 있는 버튼은 이름이 없다 — title 로는 AT 가 못 읽는다
            ...
            <div className="pt-2 border-t border-border">
              ...
              {/* bg-gray-50 은 라이트 전용 값이라 다크에서 밝은 판으로 남는다 */}
              <pre className="text-[10px] font-mono text-muted bg-surface-soft border border-border-light rounded-lg p-2.5 leading-relaxed overflow-x-auto overscroll-x-contain">
                {`<JunDSProvider
  theme="${theme}"
  colorMode="${colorMode}"
  density="${density}"
