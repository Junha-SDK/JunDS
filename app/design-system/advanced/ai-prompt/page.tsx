...
const componentGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "Modal", snippet: '<Modal open={isOpen} onClose={close}><Modal.Header>제목</Modal.Header></Modal>' },
      { id: "Toast", snippet: 'const { success } = useDsToast(); success("완료!");' },
      { id: "Tabs", snippet: '<Tabs tabs={tabs} value={tab} onChange={setTab} />' },
      { id: "Card", snippet: '<Card><Card.Header>제목</Card.Header><Card.Body>내용</Card.Body></Card>' },
      ...
      { id: "StatCard", snippet: '<StatCard label="총 수" value={142} change="+12%" trend="up" />' },
    ],
  },
  {
    group: "Patterns",
    items: [
      { id: "DataTable", snippet: '<DataTable columns={cols} data={data} rowKey={r=>r.id} selectable />' },
      { id: "FormBuilder", snippet: '<FormBuilder fields={[{name:"title",label:"제목",type:"text",required:true}]} onSubmit={handleSubmit} />' },
      { id: "ChartCard", snippet: '<ChartCard title="차트" type="bar" data={chartData} />' },
      { id: "Kanban", snippet: '<Kanban columns={columns} renderCard={renderCard} onMove={handleMove} />' },
...
- 상세 문서는 COMPONENTS.md를 참조하세요`;
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-sm text-muted mb-6">사용할 컴포넌트를 선택하면 AI에게 보낼 프롬프트를 자동 생성합니다</p>
