...
private struct DebounceStage: View {
    ...
    var body: some View {
        VStack(spacing: JdToken.Space.s5) {
            ...
            JdText(
                model.settledValue.isEmpty ? "확정된 값이 여기 나옵니다" : "확정: \(model.settledValue)",
                size: .sm, dimmed: model.settledValue.isEmpty, mono: true, lineLimit: 1)

            JdButton("초기화", variant: .secondary, size: .sm) {
                model.reset(); text = ""
            }

            Text(
                "입력이 멈추고 delay가 지나야 확정 카운터가 1 오른다 — 키를 연타하면 입력 횟수만 늘고 "
    ...
    private func counter(_ label: String, _ value: Int, dimmed: Bool) -> some View {
        VStack(spacing: JdToken.Space.s1) {
            JdText(
                "\(value)", size: .xl2, weight: JdToken.FontWeight.semibold, dimmed: dimmed,
                mono: true)
