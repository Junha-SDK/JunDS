...
enum MotionDemo {
    static let demo = ComponentDemo(
        id: "Motion",
        controls: [
            .options("preset", "preset", JdMotionPreset.allCases.map(\.rawValue), initial: "fade-in"),
...
private let motionNote = "Reduce Motion이면 전이 없이 처음부터 최종 상태다 — 내용은 즉시 보이고 "
...
private let motionDurationNote = "지속시간은 JdToken.Duration.slow(300ms)로 통일돼 있다 — 웹 값 "
    ...
    var body: some View {
        ...
        return VStack(spacing: JdToken.Space.s4) {
            ...
            VStack(spacing: JdToken.Space.s1) {
                Text(String(format: "preset=%@ · delay=%.1fs · 재생 %d회", preset.rawValue, delay, playCount))
