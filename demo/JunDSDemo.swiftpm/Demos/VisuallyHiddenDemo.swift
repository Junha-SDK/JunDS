...
private let visuallyHiddenWarning =
    "⚠️ .hidden() · isHidden = true · alpha = 0 은 대체재가 아니다 — "
    + "셋 다 접근성 트리에서도 요소를 제거한다(웹 display:none 함정과 동형). 남는 길은 크기를 0으로 "
    + "줄이는 쪽뿐이고, 0×0 요소는 VoiceOver 스와이프 순서에만 걸리므로 남용하지 않는다."

private let visuallyHiddenNote =
    "SwiftUI는 보조기술이 실제로 켜져 있을 때만 접근성 트리를 만든다 — "
...
private struct VisuallyHiddenStage: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: JdToken.Space.s4) {
            ...
            JdText(
                "보이는 것: \"\(count)\" · 낭독: \"읽지 않은 알림, \(count)개\"", size: .xs, mono: true,
                lineLimit: 2)
