...
enum NumberFormatterDemo {
    static let demo = ComponentDemo(
        ...
        controls: [
            ...
            .options(
                "format", "format", JdNumberFormatStyle.allCases.map(\.rawValue), initial: "decimal"
            ),
...
private let numberStyleNote =
    "percent는 100을 곱한다(0.15 → \"15%\") — 슬라이더 값이 크면 자릿수도 "
    + "그만큼 커진다. compact는 Foundation .compactName(\"1K\")이 아니라 Core의 천·만·억 사다리이고, "
    + "반올림 뒤 단위를 재평가한다(9999 → \"1만\")."

private let numberLocaleNote =
    "locale 기본값은 상수 \"ko-KR\"다 — 환경 의존을 막아 결정성을 지킨다. "
...
private struct NumberFormatterStage: View {
    ...
    var body: some View {
        VStack(spacing: JdToken.Space.s4) {
            // 결과 문자열이 주인공이다
            JdText(
                numberResult(state), size: .xl3, weight: JdToken.FontWeight.semibold, mono: true,
                lineLimit: 2
            )
