...
private let announcerNote = "VoiceOver가 실제로 켜져 있어야 들린다 — 꺼져 있으면 호출은 성공하지만 "
    + "아무 일도 일어나지 않는다(아래 호출 횟수만 올라간다). 웹이 같은 문구를 다시 읽히려고 노드를 "
    + "비웠다 채우던 해킹은 이식하지 않았다: iOS는 동일 문자열도 매번 다시 읽는다."

private let announcerPriorityNote = "polite는 진행 중인 발화가 끝난 뒤에 이어 읽고(.announcement), "
...
private struct AnnouncerStage: View {
    ...
    var body: some View {
        VStack(spacing: JdToken.Space.s4) {
            ...
            JdText("호출 — polite \(politeCount)회 · assertive \(assertiveCount)회", size: .sm, mono: true)
