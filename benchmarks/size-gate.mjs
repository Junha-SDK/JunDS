...
console.log(`[size] 평균: ${kb(avg)} / 예산 ${kb(budgets.componentAvgMax)} · p95: ${kb(p95)} / ${kb(budgets.componentMax)}`);
if (avg > budgets.componentAvgMax) failures.push(`W2: 평균 ${kb(avg)} > ${kb(budgets.componentAvgMax)}`);
...
if (existsSync(baselinePath) && !updateBaseline) {
  ...
  const check = (label, now, prev) => {
    if (prev && now > prev * 1.03) failures.push(`드리프트: ${label} ${kb(prev)} → ${kb(now)} (+${(((now - prev) / prev) * 100).toFixed(1)}% > 3%)`);
  };
...
console.log(`\n[size] PASS: 코어 ≤${kb(budgets.coreMax)} · 평균 ≤${kb(budgets.componentAvgMax)} · p95 ≤${kb(budgets.componentMax)}`);
