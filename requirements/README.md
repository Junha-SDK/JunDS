...
| [`book-domain`](./book-domain.md)                             | shipped | 책/독서 도메인 — 표지·책장·진행률·목차·메모·통계 + `BookReader` 패턴 + `useReadingProgress`.                  |
| [`compound-api`](./compound-api.md)                           | active  | Compound members + `asChild` Slot 위임 규약 — composite 조립 일관성.                                          |
| [`data-layer`](./data-layer.md)                               | shipped | `useResource` + `useMutation` + `useOptimisticState` — SWR-style 캐시 + invalidation + 옵티미스틱 업데이트.   |
| [`design-system-library`](./design-system-library.md)         | shipped | `@junds/ui` 라이브러리 본체 — 카테고리 구조, barrel export, polymorphic 유틸, dist 빌드.                      |
| [`forms`](./forms.md)                                         | shipped | `useForm` 자체 훅 + RHF/valibot 통합 레시피 — 작은 폼/큰 폼 두 경로 모두 권장.                                |
| [`i18n`](./i18n.md)                                           | shipped | `I18nProvider` + `defaultLocale` 사전으로 컴포넌트 내장 문자열을 부분 override.                               |
| [`layout-freedom`](./layout-freedom.md)                       | shipped | 컨테이너 기준 접힘 `Switcher` + 쇼케이스 `ResizableFrame`/Layout Lab — 레이아웃을 폭을 바꿔가며 관찰.         |
...
| [`sns-domain`](./sns-domain.md)                               | shipped | SNS 도메인 — 게시물·댓글·프로필·스토리·좋아요/팔로우·투표 + `SocialFeed` 패턴 + `useInfiniteFeed`.            |
