...
| I3 | iOS 바이너리 증가 | <1MB | **<1MB 유지** (릴리스 빌드 코드 세그먼트, 리소스 제외 · 468개 전량 기준) | 유지 |
...
- **I3 유지**: 서드파티 0 단일 SPM이므로 코드만 계상. 468개 전량 포함 후 초과가 실측되면 그때 모듈 분리(JunDSCharts 등)를 DECISIONS로 올린다 — 선제 분리는 하지 않는다.
