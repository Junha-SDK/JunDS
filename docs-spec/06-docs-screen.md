...
현행 근거: `MySelf/src/features/docs/components/pages/junds/`(JunDSLive.tsx, junds-shared.tsx, junds-usage.ts, junds-usage.data.ts, junds-live.css), `MySelf/src/styles/docs.css`의 `doc-grid--junds`. 수치 근거: 00-inventory.md(갤러리 Specimen 188 · USAGE 211키 · UI 320 + hooks 62 + finance 86 = **메타 468**).
...
1. **마스트헤드**: JunDS 로고타입 + 실측 카운터(ledger 파생: "468 components · web 320 · iOS 진행률") + 한 줄 소개. "219" 등 근거 없는 수치 금지(DEC-001).
...
- 인덱스: `CATALOG` 468건에서 마운트 후 1회 생성 — 엔트리당 `haystack = (name + id + tags.join(" ") + category + desc).toLowerCase()` 사전 연결 문자열. 468건 × ~120자 ≈ 56KB 메모리, 외부 검색 라이브러리 금지.
...
- 동시 마운트 상한 **24개**(초과 시 가장 먼 것부터 LRU 언마운트). 468개 전 마운트는 어떤 경로로도 불가능해야 한다.
