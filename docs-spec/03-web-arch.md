...
대상: UI 304개 + finance UI 86개 전량. hooks 55개는 Behavior로 전환(§5).
...
├─ components/    <jd-*> 304 + finance UI 86 — 각 폴더: element.ts + index.ts + *.css.ts
├─ behaviors/     createXxx(el, opts) 46종 (hooks 55 → 46 존속, 00-inventory §4)
...
`HTMLElement`를 직접 확장한 단일 베이스 클래스 `JdElement` 하나로 304+86개 전부를 구현한다. Lit 등 외부 베이스 채택 금지(의존성 0 원칙), 컴포넌트별 수제 `HTMLElement` 확장도 금지(반영 규칙·수명주기 관리가 390회 중복된다).
...
hooks 55개 전수의 Behavior/유틸/N-A 판정과 iOS 등가물은 **[00-inventory.md §4](./00-inventory.md)** 가 단일 소스다(존속 46 · 내부화/React 전용 9 · 중복 통합 4쌍). 이 문서에서 표를 중복 유지하지 않는다.
