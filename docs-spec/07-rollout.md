...
진행 상태의 단일 소스는 `docs-spec/registry/ledger.json`(445행)이다.

## 1. 티어 — 순서이지 범위가 아니다

최종 목표는 전량(UI 304 + Behavior 55 + finance UI 86)이다. 티어는 착수 순서만 정한다.
