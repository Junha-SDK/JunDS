...
그래서 그리기보다 계산을 먼저 세웠다. `JdChartGeometry`(Core)가 좌표를 만들고 렌더는
결과만 그린다(04 §4.2 규칙 1) — SwiftUI Canvas와 UIKit `draw(_:)`가 같은 산수를 각자
...
2. **비수치는 대입 시점에 거른다.** 좌표 하나가 NaN이면 path 전체가 **에러 없이** 사라진다
   ...
   `sanitize`를 대입 시점에 부르는 것이 계약이다.
3. **획 두께만큼 위아래를 비운다**(`inset`) — 굵은 선이 상자 경계에서 잘리지 않게.
...
**웹과 의도적으로 다른 점 하나**: 웹 v2는 색을 늘 success(초록)로 고정했다(라이브 티커
관습). 스파크라인은 방향을 보여주는 물건이라 하락을 초록으로 그리면 정보가 거꾸로 간다 —
색을 명시하지 않으면 **첫 값 대비 마지막 값**이 정한다. 명시하면 그쪽이 이긴다.
...
3. 차트 8종 — 이번 지오메트리 위에. Swift Charts가 아니라 Canvas/draw 자체 렌더로 간다
...
- **`.greatestFiniteMagnitude`가 유한하다**는 것 · 그것과 `.infinity`가 같은 결과를 줘야
  한다는 것.
- 격자는 폭을 강제하고 흐름은 자연 폭을 쓴다는 것 · 자연 폭이 컨테이너보다 넓을 때만
  강제해 다시 잰다는 것.
...
- **색은 방향과 무관하게 늘 상승색**이다(웹 라이브 티커 관습 승계). 방향은 색이 아니라
  **플래시 배경**이 말한다 — 테스트가 up/down 플래시 색이 실제로 다른지 단언한다.
...
회전 팔레트 순서는 웹과 **같게 고정**했다(cat-3→2→4→8→5). 다르면 같은 목록이 두
...
- 더 고약한 쪽: **`.greatestFiniteMagnitude`는 유한하다**(`isFinite == true`). UIKit
  관용상 `sizeThatFits`에 흔히 넘기는 값인데, `isFinite`만으로 "무제한"을 판정하면
  "폭 1.8e308으로 강제"로 읽힌다. 임계값(`unboundedThreshold`)을 두어 명시적으로 처리.

DEC-046과 같은 교훈이 반복됐다 — 측정은 **진짜 플랫폼 뷰**로 시험해야 한다. 기존
`JdWrapView` 테스트는 흐름 경로를 `sizeThatFits`를 재정의한 픽스처로만 덮고 있었고,
...
## 2026-07-27 — iOS 사용법이 독스 화면에 하나도 안 뜨던 문제 (스니펫 3 → 142)

### DEC-051. 문서를 쓴 곳과 화면이 읽는 곳이 달랐다
독스 화면(`/docs/projects/junds?c=…`)에서 컴포넌트를 열면 **iOS 탭이 전부 비어 있었다.**
원인은 문서가 없어서가 아니다 — `packages/ios/USAGE/*.md` 에 3,129줄이 이미 쓰여 있었다.
**화면이 읽는 곳은 `docs-content/*.json` 의 `snippets.swiftui/uikit` 인데 거기엔 3건뿐이었다.**

| | 전 | 후 |
|---|--:|--:|
| ios=done 행 | 142 | 142 |
| swiftui 스니펫 | 3 | **142 (100%)** |
| uikit 스니펫 | 3 | **107 (75%)** |

USAGE·RECIPES 문서를 파싱해 원장 id 로 매핑하고 `docs-content` 에 기록했다. 손으로 다시
쓰지 않은 건, 두 벌이 되면 반드시 갈라지기 때문이다 — 문서가 정본이고 `docs-content` 는
그 파생이어야 한다.

### DEC-052. 추출기가 놓친 세 가지 — 전부 표기 규약의 흔들림
1. **플랫폼 표기가 두 형식이었다.** 03·05·06 은 코드 안 주석(`// SwiftUI`), 01·02 는 코드
   밖 볼드 라벨(`**UIKit**`). 주석만 보던 첫 추출기가 01·02 의 UIKit 을 통째로 놓쳤다.
2. **마커 없이 시작하는 펜스.** `AspectRatioBox` 는 SwiftUI 코드로 시작해 끝에만 `// UIKit:`
   이 있었다. "SwiftUI 마커가 없으면 SwiftUI 아님"으로 처리해 앞부분을 버렸다.
3. **`Jd` 접두 없는 섹션.** `### Toast` 는 헤딩 정규식(`Jd[A-Za-z]+`)에 걸리지 않았다.

교훈: 문서에서 기계적으로 뽑아 쓸 생각이면 **표기 규약을 먼저 하나로 고정**해야 한다.
지금은 추출기가 세 형식을 모두 받아 주지만, 새 USAGE 문서는 코드 안 주석으로 통일한다.

### DEC-053. 별칭은 정본 스니펫을 재사용하고 그 사실을 첫 줄에 밝힌다
`AspectRatio`·`ConfirmDialog`·`Sheet`·`CoreDivider`·`Switch`·`OTPInput`·`Group` 은 원장이
이미 "별칭"이라 적어 둔 것들이다. 각자 예제를 새로 쓰면 정본과 갈라지므로, 정본 스니펫을
그대로 쓰되 첫 줄에 `// Sheet 는 BottomSheet 별칭이다` 를 붙였다. 읽는 사람이 "왜 다른
타입 이름이 나오지?" 하고 멈추지 않게 하기 위해서다.

### DEC-054. UIKit 이 비어 있는 37건은 채우지 않는다
훅 23종은 iOS 대응이 Core 타입(`JdDebouncer`·`JdForm`)이나 시스템 API(`UIPasteboard`·
`NWPathMonitor`)라 두 계층이 같은 코드다. 같은 줄을 UIKit 탭에 복사하면 정보가 아니라
소음이다. `Show`/`Hide` 는 `.jdShow`/`.jdHide` 모디파이어라 **SwiftUI 전용**이 사실이고,
`Page`·`Section`·`Container`·`Overlay`·`Grid` 는 RECIPES 가 SwiftUI 레시피만 정의한다.

**남은 진짜 숙제는 화면 쪽이다.** 빈 탭이 "미구현"인지 "이 플랫폼엔 개념이 없음"인지
"양 계층이 동일"인지 구분되지 않는다 — 사용자가 이번에 본 것도 그 빈 화면이었다.
`06-docs-screen` 에 빈 상태 문구 규칙을 넣어야 한다(후속).

### 아직 남은 더 큰 구멍
**web 스니펫은 445 done 중 51건(11%)뿐이다.** 이번 패스는 사용자가 지목한 iOS 만 다뤘다.
같은 방식으로 `packages/web` 의 데모·README 에서 뽑아 채울 수 있다.

결정자: 사용자 지적(독스 화면에 iOS 설명 부재) + 추출 방식·별칭 처리는 기본값 채택.
...
**옮기지 않기로 했다.** 이 화면들의 부피 대부분은 도메인 집계와 데이터 모델 결합이지
재사용 가능한 UI 가 아니다. 대신 **화면이 쓰던 재료**를 골라냈다:

- `NowPlayingFull`(187줄)에 묻혀 있던 가사 로직 → `Lyrics`
- `DailyStats`(212줄)에 묻혀 있던 `BarList` → `BarList`
- `GlobeWireframe`(131줄)의 전체 창 캔버스 → 크기 지정형 `GlobeWireframe`

판단 기준: **"다른 데이터로 다시 쓸 수 있는가."** 가사 뷰는 어떤 곡에도 쓰이고, 막대
...
`Modal` 에 className 을 덮어써서 조립했다. 배경 워시 같은 시각적 특성은 자식이 그리고,
a11y 기계장치는 `Modal` 이 계속 소유한다. 오버레이 접근성 버그를 고칠 곳이 한 군데로
...
어느 쪽이 옳다고 정하는 대신 **prop 으로 노출했다**(`wpm`·`cpm`·`minMinutes`). 읽기 속도는
글의 성격과 독자층에 달린 튜닝값이지 디자인 시스템이 정할 상수가 아니다. 기본값은 JunDS
쪽을 유지해 기존 사용처의 표시가 바뀌지 않게 했고, MySelf 는 세 값을 넘겨 현재 숫자를
그대로 재현할 수 있다.
...
결정자: 사용자 요청("다른것도 이어서 다해줘") + 재료/화면 경계는 기본값 채택.
...
- `intrinsicContentSize`는 내용 리프의 것이다. **내부 제약으로 크기가 정해지는 컨테이너**는
  `noIntrinsicMetric`을 돌려준다.

`JdMicroKpiCellView`가 정확히 후자였다(내부 스택을 네 변에 핀). 두 경로 모두 0 → 높이 0.
...
교훈: 측정·배치처럼 **프레임워크 기본 구현에 의존하는 코드**는 픽스처가 아니라 진짜
플랫폼 뷰로 테스트해야 한다.
...
- **`JdMeasure` 신설** — 측정 규칙을 한 곳에. `systemLayoutSizeFitting` → `sizeThatFits`
  ...
  없어 첫 경로가 0을 주므로 자연히 두 번째로 내려간다. 반대 순서면 결함이 그대로 남는다.
- `JdWrapView`(2곳)·`JdColumnsView`(1곳)의 측정을 전부 위임.
- **회귀 테스트를 진짜 Auto Layout 뷰로**: `sizeThatFits`도 `intrinsicContentSize`도
  재정의하지 않는 `AutoLayoutCard`를 만들어, 전제(둘 다 0을 준다)까지 함께 단언한다 —
...
- `LiveStackedCell` UIKit 스테이지를 **열 정렬 표**로 바꿔(셀의 실제 서식지) 확인:
  ...
  가격 열(`.fixed(96, align: .end)`)에서 `71,200`/`8,240`/`168,500`의 **오른쪽 끝이 맞음** ·
  위치 막대 열이 세 행 모두 같은 x에서 시작·종료 · SK의 `0.00%`가 초록(`.gainOrEven`).
