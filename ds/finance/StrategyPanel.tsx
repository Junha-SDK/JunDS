...
import { strategyFor, type StrategyLevel, type ScoreBreakdown } from "./lib/strategy";
import { AppIcon } from "./AppIcon";

// REC 5단계 톤 — 가격 의미론(up=빨강/down=파랑) 토큰 기반
const RECO_TONE: Record<
  string,
  { color: string; bg: string; icon: "trendingUp" | "trendingDown" | "activity" }
> = {
  강력매수: {
    color: "var(--bm-up)",
    ...
    <section className="bm-card overflow-hidden" style={{ border: "1px solid var(--bm-border)" }}>
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4">
        ...
        >
          ...
          {/* 검정 반투명 구분선은 다크 표면 위에서 사라진다 — 테두리 토큰을 쓴다 */}
          <div className="text-right pr-3" style={{ borderRight: "1px solid var(--bm-border)" }}>
            <div className="text-[10.5px] text-[color:var(--bm-muted)] font-bold">추천 점수</div>
            <div className="bm-num font-extrabold text-[16px]" style={{ color: tone.color }}>
              {strategy.recommendationScore.toFixed(0)}
              <span
                className="text-[10.5px] font-semibold ml-0.5"
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-3">
          ...
          <KpiTile
            label="B1까지"
            value={`${distToBuy >= 0 ? "+" : ""}${distToBuy.toFixed(2)}`}
            unit="%"
            tone={distToBuy <= 0 ? "buy" : "neutral"}
          />
          <KpiTile
            label="손절까지"
            value={`${distToStop.toFixed(2)}`}
            unit="%"
            tone={distToStop > 5 ? "buy" : "stop"}
          />
        </div>
      </div>
      ...
      <div className="px-4 pb-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <ZoneColumn
          title="익절 구간"
          tone="sell"
          levels={strategy.takeProfitZones}
          current={price}
        />
      ...
      >
        <PositionTile
          label="안전형"
          pct={strategy.positionSize.conservative}
          color="var(--bm-info)"
        />
        <PositionTile
          label="균형형"
          pct={strategy.positionSize.balanced}
...
function KpiTile({
  ...
  return (
    <div className="rounded-xl px-3 py-2 text-center" style={{ background: "var(--bm-soft-100)" }}>
    ...
    <div className="space-y-2">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="text-right bm-num">
            <div className="font-extrabold text-[18px]" style={{ color: "var(--bm-down)" }}>
              {level.price.toLocaleString("ko-KR")}
            </div>
            <div className="text-[10.5px] font-semibold" style={{ color: "var(--bm-muted)" }}>
              {dist.toFixed(2)}% 거리
        ...
        <p
          className="text-[12px] mt-2 font-semibold leading-relaxed"
...
function ReasonPanel({
  ...
  return (
    <details className="px-4 pb-3" style={{ borderTop: "1px solid var(--bm-border)" }}>
      {/* summary 는 실제로 포커스를 받는 요소다 — list-none 으로 기본 마커까지 지운 이상
          포커스 표시가 없으면 키보드로는 이 접힘을 찾을 방법이 없다. */}
      <summary
        className={[
          "cursor-pointer list-none py-3 flex items-center gap-2 text-[12.5px] font-extrabold select-none rounded-lg",
          "transition-colors hover:bg-[color:var(--bm-soft-100)]",
      ...
      >
        ...
        <span
          className="ml-auto shrink-0 text-[10.5px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap tabular-nums"
      ...
      <div className="mt-1 grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-3">
        ...
        >
          ...
          <ul className="space-y-1.5">
            ...
            />
            <BreakdownRow
              label={`등락률 ${breakdown.changePct >= 0 ? "+" : ""}${breakdown.changePct.toFixed(
                2,
              )}% × ${breakdown.changeWeight}`}
              detail="당일 시세 모멘텀이 강할수록 가산"
              value={`${
                breakdown.changeContribution >= 0 ? "+" : ""
              }${breakdown.changeContribution.toFixed(1)}점`}
            ...
            <BreakdownRow
              ...
              value={`${breakdown.rrContribution >= 0 ? "+" : ""}${breakdown.rrContribution.toFixed(
                1,
              )}점`}
          ...
          <div
            ...
            style={{ borderTop: "1px solid var(--bm-border)", color: "var(--bm-muted)" }}
          >
            <strong className="font-extrabold" style={{ color: "var(--bm-text)" }}>
              판정 기준:
            </strong>{" "}
        ...
        >
          ...
          <p className="mb-2.5">
            신뢰도는 추천 점수가 중립값(50점)에서 얼마나 멀리 떨어져 있는지를 0.45~0.95 범위로
            환산한 값입니다. 점수가 양극단(0 또는 100)에 가까울수록 라벨에 대한 확신이 높습니다.
          </p>
          ...
          >
            ⚠ 본 점수는 데모 알고리즘(등락률 + 손익비 가중합)으로 산출된 참고용 신호이며, 투자
            권유가 아닙니다.
          </div>
        </div>
      </div>
