...
import { BottomSheet, Button, Input, useDsToast } from "@junds/ui";
...
export function AlertSheet({ open, onClose, name }: AlertSheetProps) {
  ...
  const [value, setValue] = useState<string>(() => Math.round(tick.price * 1.05).toString());
  const toast = useDsToast();

  const numeric = Number(value.replace(/,/g, ""));
  const valid = Number.isFinite(numeric) && numeric > 0;
  const delta = valid && tick.price > 0 ? ((numeric - tick.price) / tick.price) * 100 : 0;
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="가격 알림 등록" height="auto">
      <div className="px-4 pb-5 pt-1">
        <div className="text-[13px] text-[color:var(--bm-muted)] font-semibold">종목</div>
        ...
        <div className="mt-1 flex items-baseline gap-2 text-[12.5px]">
          ...
          <span className="bm-num font-bold">{tick.price.toLocaleString("ko-KR")}</span>
        </div>

        <div className="mt-4">
          <div className="text-[12px] font-bold text-[color:var(--bm-muted)] mb-1.5">조건</div>
          <SegmentedPill
            options={[
              { key: "above", label: "이상 ↑" },
              { key: "below", label: "이하 ↓" },
        ...
        <div className="mt-4">
          <div className="text-[12px] font-bold text-[color:var(--bm-muted)] mb-1.5">목표가</div>
          <Input
            size="lg"
            inputMode="numeric"
            value={value}
          ...
          <div className="mt-2 flex items-center gap-2">
            {[0.95, 1.0, 1.05, 1.1, 1.2].map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => setValue(Math.round(tick.price * m).toString())}
                // finance 표면은 --bm-* 어휘를 쓴다 — 포커스 링도 그 어휘로 그려야 겉돌지 않는다
                className="bm-pill text-[11px] font-bold cursor-pointer transition-opacity hover:opacity-80 active:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--bm-accent-strong)] focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--bm-card)]"
                style={{
