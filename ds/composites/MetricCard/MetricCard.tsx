      ...
      <div ref={ref} className={cn("rounded-xl border border-border bg-white p-5", className)}>
        ...
        {change !== undefined && (
          <div className="flex items-center gap-1.5 mt-2">
            <span
              className={cn("text-xs font-semibold", isPositive ? "text-success" : "text-danger")}
            >
        ...
        {sparkline &&
          ...
          (() => {
            ...
            const points = sparkline
              ...
              .join(" ");
            const areaPoints = `${pad},${h - pad} ${points} ${
              pad + (sparkline.length - 1) * step
            },${h - pad}`;
            return (
              <svg className="w-full mt-3" viewBox={`0 0 ${w} ${h}`} style={{ height: 48 }}>
                ...
                />
                <circle
                  cx={pad + (sparkline.length - 1) * step}
                  cy={
                    h -
                    pad -
                    ((sparkline[sparkline.length - 1] - sparkMin) / sparkRange) * (h - pad * 2)
                  }
                  r="3"
                  fill="var(--primary)"
                />
