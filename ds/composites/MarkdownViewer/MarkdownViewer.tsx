...
function parseMarkdown(md: string, breaks: boolean): string {
  const html = md
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    // bg-gray-100 은 라이트 전용이라 다크에서 코드 칩이 흰 덩어리로 남는다
    .replace(
      /`(.+?)`/g,
      '<code class="px-1.5 py-0.5 bg-primary-light border border-primary/15 rounded-lg text-xs font-mono text-primary-ink">$1</code>',
    )
    .replace(/^\- (.+)$/gm, '<li class="ml-4 list-disc">$1</li>')
    .replace(/^\d+\. (.+)$/gm, '<li class="ml-4 list-decimal">$1</li>')
    .replace(
      /\[(.+?)\]\((.+?)\)/g,
      '<a href="$2" class="text-primary-ink underline" target="_blank" rel="noopener">$1</a>',
    )
    .replace(
      /^> (.+)$/gm,
      '<blockquote class="pl-4 border-l-4 border-primary/30 text-muted italic my-2">$1</blockquote>',
    )
