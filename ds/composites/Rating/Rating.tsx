...
export const Rating = forwardRef<HTMLDivElement, RatingProps>(
  ({ value, onChange, max = 5, half = false, size = "md", disabled, readOnly, className }, ref) => {
    // SVG gradient id 는 문서 전역이다. `half-0` 같은 고정 이름을 쓰면 한 화면에 Rating 이
    // 둘 있을 때 뒤 인스턴스의 그라디언트가 앞을 덮어쓴다 — 인스턴스마다 useId 로 가른다
        ...
        {Array.from({ length: max }, (_, i) => {
          ...
          return (
            <span
              key={i}
              className={cn(
                "relative inline-flex rounded-md transition-transform duration-150 motion-reduce:transition-none",
                // tabIndex 가 붙어 실제로 초점을 받는 요소다 — 링을 걸 자리가 여기다
                "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55 focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                interactive &&
                  "cursor-pointer hover:scale-110 active:scale-95 motion-reduce:hover:scale-100 motion-reduce:active:scale-100",
            ...
            >
              <svg width={starSize} height={starSize} viewBox="0 0 24 24" fill="none">
                {halfFilled && (
                  <defs>
                    <linearGradient id={`${gradientId}-half-${i}`}>
                ...
                <path
                  ...
                  // 빈 별을 검정 4% 로 채우면 다크 배경에서 사라진다 — 모드를 따라가는 변수로
                  fill={
                    filled
                      ? "var(--warning)"
                      : halfFilled
                      ? `url(#${gradientId}-half-${i})`
                      : "var(--border-light)"
                  }
