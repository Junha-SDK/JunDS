...
const ringMap: Record<StoryRingState, string> = {
  ...
  read: "bg-gray-300 dark:bg-gray-700 p-[2px]",
  live: "bg-gradient-to-tr from-rose-500 to-rose-700 p-[2.5px]",
  muted: "bg-gray-200 dark:bg-gray-800 p-[2px]",
};
...
export const StoryCircle = forwardRef<HTMLButtonElement, StoryCircleProps>(
  ({ name, avatar, state = "unread", size = 64, onClick, className }, ref) => (
    <button
      ...
      aria-label={`${name} 스토리`}
      className={cn(
        "inline-flex flex-col items-center gap-1 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 rounded-md p-1",
        className,
      )}
    >
      <div className={cn("rounded-full inline-flex items-center justify-center relative", ringMap[state])} style={{ width: size, height: size }}>
        <div className="rounded-full bg-surface w-full h-full p-[2px]">
          {avatar ? (
            <img src={avatar} alt="" className="w-full h-full rounded-full object-cover" />
          ) : (
            <div className="w-full h-full rounded-full bg-primary/15 text-primary flex items-center justify-center text-sm font-bold">
              {name.slice(0, 1)}
            </div>
          )}
        </div>
