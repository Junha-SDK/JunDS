      ...
      <div className={cn("inline-flex bg-gray-100 rounded-lg p-0.5 gap-0.5", className)} role="tablist">
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              "inline-flex items-center gap-1.5 font-medium transition-all duration-150 rounded-md cursor-pointer",
      ...
      <div className={cn("inline-flex gap-1", className)} role="tablist">
        {tabs.map((tab) => (
          <button
            ...
            onClick={() => onChange(tab.value)}
            className={cn(
              "inline-flex items-center gap-1.5 font-medium rounded-full transition-all duration-150 cursor-pointer",
              "disabled:opacity-40 disabled:cursor-not-allowed",
    ...
    <div className={cn("flex border-b border-border gap-0", className)} role="tablist">
      {tabs.map((tab) => (
        <button
          ...
          onClick={() => onChange(tab.value)}
          className={cn(
            "inline-flex items-center gap-1.5 font-medium transition-all duration-150 border-b-2 -mb-px cursor-pointer",
            "disabled:opacity-40 disabled:cursor-not-allowed",
