 ...
 *
 * v2 원본: ds/finance/lib/kis.ts (시그니처·동작 동일 이관 — DEC-003 분리).
 *
 ...
 * 환경변수에 의존하므로 **반드시 서버(route handler / Server Action / 백엔드)
 * 에서만 import**해야 한다. 이 때문에 패키지 배럴(index)에서 제외되며
 * 서브패스(`@junds/finance-data/kis`)로만 노출된다.
 *
 * 클라이언트 번들에 포함되려는 경우는 번들러가 `node:fs` 해석 실패로 빌드
 * 단계에서 막는다. 우회 빌드 설정 등 클라이언트에서 평가되는 시나리오를
 * 막기 위해 모듈 평가 시점에 `window` 가 있으면 즉시 throw 한다.
 */
...
/**
 * 토큰 디스크 캐시 — dev 모드 hot-reload 시 모듈이 재평가돼 메모리 캐시가
 * 날아가는데, KIS는 토큰 발급을 1분당 1회로 제한하므로 디스크에 보관해
 * 재발급 폭주를 막는다. 파일명은 v2와 동일하게 유지해 전환기 동안 v2 셤과
 * 캐시를 공유한다. (`/tmp/buttermoney-kis-token.json`)
 */
...
/**
 * KIS 실전 REST는 초당 ~5건 제한. 여러 호출부가 병렬로 요청해도 차단되지 않도록
...
export function kisCodeFor(input: string): string | null {
  ...
  if (tickerMatch) return tickerMatch[1] ?? null;
 ...
 * ```json
 * {
 *   "symbol": "삼성전자",          // string — 종목명 (입력값 그대로)
 *   "code": "005930",             // string — 6자리 종목코드
 *   "market": "KOSPI",            // "KOSPI" | "KOSDAQ" | "UNKNOWN"
 *   "price": 71200,               // number — 현재가 (원)
 *   "change": 1200,               // number — 전일대비 (원)
 *   "changePct": 1.71,            // number — 전일대비 (%)
 *   "open": 70100, "high": 71600, "low": 69900, "prevClose": 70000,
 *   "volume": 12345678,           // number — 누적거래량 (주)
 *   "amount": 876543210000,       // number — 누적거래대금 (원)
 *   "marketCap": 425000000000000, // number — 시가총액 (원)
 *   "per": 12.3, "pbr": 1.4, "eps": 5788, "bps": 50817,   // number | null
 *   "high52": 88800, "low52": 49900,                       // number | null
 *   "foreignOwnership": 51.2,     // number | null — 외국인 보유율 (%)
 *   "asOf": "2026-07-24T05:30:00.000Z"  // string — ISO 8601
 * }
 * ```
 */
 ...
 * ```json
 * {
 *   "date": "2026-07-24",   // string — YYYY-MM-DD
 *   "open": 70100, "high": 71600, "low": 69900, "close": 71200,  // number (원)
 *   "volume": 12345678,     // number (주)
 *   "changePct": 1.71       // number (%)
 * }
 * ```
 */
...
export async function fetchChart(
  ...
  return rows
    ...
    .map<KisCandle>((r) => {
      const d = r.stck_bsop_date as string; // YYYYMMDD (위 filter로 존재 보장)
...
/**
 * 호가창 스냅샷 (매도/매수 1~10단계).
 *
 * JSON 스키마:
 * ```json
 * {
 *   "symbol": "삼성전자", "code": "005930",
 *   "current": 71200,                                // number — 현재가 (원)
 *   "asks": [{ "price": 71300, "qty": 1200 }],       // 10개 — 1번이 최우선
 *   "bids": [{ "price": 71200, "qty": 800 }],        // 10개 — 1번이 최우선
 *   "totalAskQty": 45000, "totalBidQty": 39000,      // number (주)
 *   "asOf": "2026-07-24T05:30:00.000Z"
 * }
 * ```
 */
 ...
 * ```json
 * {
 *   "foreign": 1234,       // number — 외국인 순매수 (억원, +매수 / -매도)
 *   "institution": -567,   // number — 기관 순매수 (억원)
 *   "individual": 0,       // number — 개인 순매수 (억원; 이 TR은 미포함 → 항상 0)
 *   "asOf": "2026-07-24T05:30:00.000Z"
 * }
 * ```
 */
 ...
 * ```json
 * {
 *   "code": "KOSPI",        // "KOSPI" | "KOSDAQ" | "KOSPI200"
 *   "value": 7402.77,       // number — 현재 지수
 *   "change": -12.3,        // number — 전일대비 (pt)
 *   "changePct": -0.17,     // number — 전일대비 (%)
 *   "open": 7410.1, "high": 7431.9, "low": 7388.2, "prevClose": 7415.07,
 *   "asOf": "2026-07-24T05:30:00.000Z"
 * }
 * ```
 */
