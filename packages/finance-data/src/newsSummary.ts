 ...
 *
 * v2 원본: ds/finance/lib/newsSummary.ts (시그니처·동작 동일 이관).
 * rss.ts가 가져온 항목의 후처리 — 뉴스 데이터 파이프라인의 일부라 함께 이관.
 *
 * Approach:
 ...
 *
 * Server-safe: pure functions. Inputs are news items already fetched.
 */
 ...
 * ```json
 * {
 *   "summary": "문장1 문장2",                 // string — 선택 문장 join
 *   "sentences": ["문장1", "문장2"],          // string[]
 *   "keyTerms": [{ "term": "반도체", "count": 7 }],  // 상위 8개
 *   "tone": 0.33,                            // number — [-1, 1]
 *   "itemCount": 12                          // number — 기여 기사 수
 * }
 * ```
 */
