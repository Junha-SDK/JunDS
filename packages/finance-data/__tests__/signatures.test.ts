...
const _fetchQuote: (name: string, venue?: "J" | "NX" | "UN") => Promise<KisQuote | null> =
  fetchQuote;
const _fetchChart: (
  name: string,
  period?: ChartPeriod,
...
const _fetchAllFeeds: (specs?: RssFeedSpec[], revalidate?: number) => Promise<RssItem[]> =
  fetchAllFeeds;
const _feedSpecs: RssFeedSpec[] = FEED_SPECS;

/* ── v2 ds/finance/lib/newsSummary.ts ── */
const _summarizeNews: (items: SummarizableNews[], maxSentences?: number) => NewsSummary =
  summarizeNews;
...
const _seedTick: (name: string, price: number, changePct: number, venue?: TickVenue) => void =
  seedTick;
