...
describe("jd-typewriter 동적 소스", () => {
  test("once 완료 뒤 text를 바꾸면 새 문장을 처음부터 다시 타이핑한다", async () => {
    ...
    try {
      document.body.innerHTML =
        `<jd-typewriter text="이전" speed="1" once></jd-typewriter>`;
      await tick();
      const typewriter =
        document.querySelector<JdTypewriter>("jd-typewriter")!;
      const typed = typewriter.querySelector<HTMLElement>(
      ...
      expect(
        typewriter.querySelector(".jd-typewriter__sr")!.textContent,
      ).toBe("새문장");
