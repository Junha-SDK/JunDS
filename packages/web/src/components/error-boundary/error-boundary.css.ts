...
@layer junds.components {
  jd-error-boundary { display: block; }

  .jd-error-boundary__fallback {
    display: none; flex-direction: column; align-items: center; justify-content: center;
    gap: var(--jd-space-1); padding: var(--jd-space-8); box-sizing: border-box;
    text-align: center; font-family: var(--jd-font-sans);
    border: var(--jd-border-thin) solid color-mix(in srgb, var(--jd-color-danger) 20%, transparent);
    border-radius: var(--jd-radius-xl);
    background: var(--jd-color-danger-light);
  }
  /* 실패 시에만 폴백 노출 + 나머지 children 은닉 */
  jd-error-boundary[failed] > .jd-error-boundary__fallback { display: flex; }
  jd-error-boundary[failed] > *:not(.jd-error-boundary__fallback) { display: none; }
  ...
  .jd-error-boundary__message {
    margin: 0 0 var(--jd-space-2); font-size: var(--jd-text-xs);
    color: var(--jd-color-muted);
  }
  .jd-error-boundary__message[hidden] { display: none; }
  ...
  .jd-error-boundary__retry:hover { background: var(--jd-color-danger-hover); }
  .jd-error-boundary__retry:focus-visible {
    outline: none; box-shadow: var(--jd-shadow-focus-ring-danger);
  }

  @media (prefers-reduced-motion: reduce) {
    .jd-error-boundary__retry { transition: none; }
  }
}`;
