...
@layer junds.components {
  ...
  jd-pagination[hidden] { display: none; }

  .jd-pagination__list {
    display: flex; align-items: center; gap: var(--jd-space-1);
    margin: 0; padding: 0; list-style: none;
  }
  .jd-pagination__item { display: inline-flex; }
  ...
  .jd-pagination__page:focus-visible { outline: none; box-shadow: var(--jd-shadow-focus-ring); }
  .jd-pagination__arrow:disabled { opacity: var(--jd-opacity-30); pointer-events: none; }
  .jd-pagination__arrow > svg { inline-size: 16px; block-size: 16px; }
  ...
  .jd-pagination__ellipsis { cursor: default; user-select: none; }

  @media (prefers-reduced-motion: reduce) {
    .jd-pagination__arrow, .jd-pagination__page { transition: none; }
  }
}`;
