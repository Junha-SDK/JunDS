...
@layer junds.components {
  ...
  .jd-date-input__input::-webkit-calendar-picker-indicator { cursor: pointer; }

  jd-date-input[error] > .jd-date-input__input { border-color: var(--jd-color-danger); }
  ...
  .jd-date-input__clear {
    position: absolute; inset-inline-end: 2rem; bottom: .5rem;
  ...
  .jd-date-input__clear:hover { color: var(--jd-color-foreground); }
  .jd-date-input__clear:focus-visible {
    outline: none; color: var(--jd-color-foreground);
    box-shadow: var(--jd-shadow-focus-ring);
  }
  .jd-date-input__clear[hidden] { display: none; }

  @media (prefers-reduced-motion: reduce) {
    .jd-date-input__input, .jd-date-input__clear { transition: none; }
  }
}`;
