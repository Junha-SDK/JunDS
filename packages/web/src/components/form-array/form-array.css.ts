...
@layer junds.components {
  jd-form-array { display: block; }
  jd-form-array > template { display: none; }
  ...
  .jd-form-array__control { flex: 1; min-width: 0; }

  .jd-form-array__remove {
    flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center;
    margin-block-start: var(--jd-space-1);
    padding: var(--jd-space-2); border: 0; background: none;
  ...
  .jd-form-array__add[hidden] { display: none; }
  ...
  .jd-form-array__add > svg { flex-shrink: 0; }

  @media (prefers-reduced-motion: reduce) {
    .jd-form-array__remove, .jd-form-array__add { transition: none; }
  }
}`;
