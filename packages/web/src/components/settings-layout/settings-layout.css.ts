...
@layer junds.components {
  ...
  .jd-settings-layout__title[hidden] { display: none; }
  ...
  .jd-settings-layout__icon[hidden] { display: none; }
  .jd-settings-layout__icon svg { width: 1rem; height: 1rem; }
  ...
  .jd-settings-layout__panel[hidden] { display: none; }

  @media (min-width: 1024px) {
    jd-settings-layout { flex-direction: row; }
    .jd-settings-layout__sidebar {
      width: var(--_jd-settings-sidebar-w, 220px);
      min-width: var(--_jd-settings-sidebar-w, 220px);
      border-bottom: 0;
      border-right: var(--jd-border-thin) solid var(--jd-color-border);
    }
    .jd-settings-layout__content { padding: var(--jd-space-8); }
  }

  @media (prefers-reduced-motion: reduce) {
    .jd-settings-layout__item { transition: none; }
  }
}`;
