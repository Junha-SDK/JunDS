...
@layer junds.components {
  jd-alert {
    ...
    box-sizing: border-box; padding: var(--jd-space-4);
    font-family: var(--jd-font-sans); color: var(--jd-color-foreground);
    border: var(--jd-border-thin) solid color-mix(in srgb, var(--_jd-alert-color) 20%, transparent);
  ...
  jd-alert[variant="success"] { --_jd-alert-color: var(--jd-color-success); }
  jd-alert[variant="warning"] { --_jd-alert-color: var(--jd-color-warning); }
  jd-alert[variant="danger"] { --_jd-alert-color: var(--jd-color-danger); }

  .jd-alert__icon {
    display: flex; flex-shrink: 0; margin-block-start: var(--jd-space-px);
    color: var(--_jd-alert-color);
  }
  .jd-alert__body { flex: 1; min-width: 0; }
  .jd-alert__title {
  ...
  .jd-alert__title[hidden] { display: none; }
  .jd-alert__content { font-size: var(--jd-text-sm); line-height: var(--jd-leading-relaxed); }
  ...
  .jd-alert__close:hover { color: var(--jd-color-foreground); }
  .jd-alert__close[hidden] { display: none; }
  .jd-alert__close:focus-visible { outline: none; box-shadow: var(--jd-shadow-focus-ring); }

  @media (prefers-reduced-motion: reduce) { .jd-alert__close { transition: none; } }
}`;
