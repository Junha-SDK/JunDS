...
@layer junds.components {
  jd-otp-input { display: inline-flex; align-items: center; gap: var(--jd-space-1-5); }

  .jd-otp-input__separator {
    width: 0.75rem; height: 2px; margin-inline: var(--jd-space-1);
    background: var(--jd-color-border); border-radius: var(--jd-radius-full);
  }

  .jd-otp-input__cell {
    box-sizing: border-box; width: 2.75rem; height: 3.25rem; margin: 0; padding: 0;
    ...
    border-radius: var(--jd-radius-xl); outline: none;
  ...
  .jd-otp-input__cell:disabled { opacity: var(--jd-opacity-50); cursor: not-allowed; }
  ...
  jd-otp-input[error] .jd-otp-input__cell:focus { box-shadow: var(--jd-shadow-focus-ring-danger); }
  ...
  @media (prefers-reduced-motion: reduce) {
    .jd-otp-input__cell { transition: none; }
    .jd-otp-input__cell:focus { transform: none; }
    jd-otp-input[error] .jd-otp-input__cell { animation: none; }
  }
}`;
