...
@layer junds.components {
  jd-emoji-reaction {
    display: inline-flex; align-items: center; flex-wrap: wrap; gap: var(--jd-space-1);
  ...
  .jd-emoji-reaction__item {
    ...
    font-size: var(--jd-text-xs); line-height: var(--jd-leading-none);
    cursor: pointer;
  ...
  .jd-emoji-reaction__item:hover { background: var(--jd-color-card-hover); }
  ...
  .jd-emoji-reaction__emoji { font-size: var(--jd-text-sm); line-height: var(--jd-leading-none); }
  ...
  .jd-emoji-reaction__add {
    ...
    width: 28px; height: 24px; box-sizing: border-box; /* 테두리 포함 28×24 유지(§6) */
    border-radius: var(--jd-radius-full);
    border: var(--jd-border-thin) solid var(--jd-color-border);
    background: var(--jd-color-card);
    color: var(--jd-color-muted);
    font-size: var(--jd-text-sm); cursor: pointer;
    transition: background var(--jd-duration-fast) var(--jd-easing-ease-out),
                color var(--jd-duration-fast) var(--jd-easing-ease-out);
  }
  ...
  .jd-emoji-reaction__add[hidden] { display: none; }
}`;
