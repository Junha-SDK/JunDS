...
@layer junds.components {
  ...
  jd-rating[disabled] { opacity: var(--jd-opacity-50); }
  ...
  jd-rating[size="sm"] .jd-rating__star { width: 1rem; height: 1rem; }
  jd-rating[size="lg"] .jd-rating__star { width: 1.75rem; height: 1.75rem; }
  ...
  .jd-rating__icon--fill {
    fill: var(--jd-color-warning); stroke: var(--jd-color-warning);
    clip-path: inset(0 100% 0 0); /* data-fill="none" */
  }
  .jd-rating__star[data-fill="half"] .jd-rating__icon--fill { clip-path: inset(0 50% 0 0); }
  .jd-rating__star[data-fill="full"] .jd-rating__icon--fill { clip-path: inset(0); }
  /* v2: 반쯤 찬 별도 외곽선 전체가 warning이다 */
  .jd-rating__star[data-fill="half"] .jd-rating__icon--empty,
  .jd-rating__star[data-fill="full"] .jd-rating__icon--empty { stroke: var(--jd-color-warning); }

  /* 히트 영역 — 아이콘 위에 깔린다(뒤 형제라 z-index 불필요) */
  .jd-rating__hit {
    position: absolute; inset-block: 0; width: 50%; cursor: pointer;
  }
  .jd-rating__hit[data-side="half"] { inset-inline-start: 0; }
  .jd-rating__hit[data-side="full"] { inset-inline-end: 0; }
  /* 반칸이 아니면 한 칸이 별 전체를 덮는다 */
  jd-rating:not([half]) .jd-rating__hit[data-side="full"] { inset-inline: 0; width: 100%; }
  jd-rating[readonly] .jd-rating__hit,
  jd-rating[disabled] .jd-rating__hit { cursor: default; }
