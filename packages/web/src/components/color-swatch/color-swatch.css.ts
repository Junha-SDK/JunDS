...
@layer junds.components {
  ...
  .jd-color-swatch__item:hover { transform: scale(1.05); }
  ...
  jd-color-swatch[size="sm"] .jd-color-swatch__item { width: 1.5rem; height: 1.5rem; }
  jd-color-swatch[size="lg"] .jd-color-swatch__item { width: 2.5rem; height: 2.5rem; }
  ...
  .jd-color-swatch__label[hidden] { display: none; }

  @media (prefers-reduced-motion: reduce) {
    .jd-color-swatch__item { transition: none; }
    .jd-color-swatch__item:hover,
    .jd-color-swatch__item[data-selected] { transform: none; }
  }
}`;
