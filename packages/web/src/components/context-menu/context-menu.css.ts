...
export default css`
@layer junds.base {
  jd-context-menu:not(:defined) { display: block; }
  jd-context-menu:not(:defined) > script { display: none; }
}
@layer junds.components {
  /* 우클릭 영역을 감싸는 블록 — v2의 <div onContextMenu> 자리 */
  jd-context-menu { display: block; position: relative; }
  jd-context-menu > .jd-popover__trigger { display: block; }
  jd-context-menu > .jd-popover__trigger[tabindex] { display: block; }
  ...
  jd-context-menu > .jd-popover__panel {
    ...
    margin: 0;
    --jd-popover-tx: 0; --jd-popover-ty: 0;
    min-width: 11.25rem;
  ...
  jd-context-menu .jd-dropdown__item { padding-block: var(--jd-space-2); }
  jd-context-menu .jd-dropdown__divider { background: var(--jd-color-border-light); }
}`;
