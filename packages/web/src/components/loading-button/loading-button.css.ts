...
@layer junds.components {
  jd-loading-button { display: inline-flex; }
  jd-loading-button[full-width] { display: flex; }
  jd-loading-button[full-width] > .jd-button { width: 100%; }
  ...
  jd-loading-button[variant="danger"] > .jd-button:active { filter: brightness(.95); scale: .98; }
  ...
  jd-loading-button[variant="link"] > .jd-button:active { scale: none; }

  /* 스피너 크기 분기(md·lg는 .jd-button__spinner base가 16px) */
  jd-loading-button[size="xs"] .jd-button__spinner { width: 12px; height: 12px; }
  jd-loading-button[size="sm"] .jd-button__spinner { width: 14px; height: 14px; }

  /* 라벨 교체 — 보이는 칸이 곧 접근 이름이다(display:none은 a11y 트리에서도 제외).
     data-swap = loadingText가 있을 때만 켜진다(없으면 v2처럼 children 유지). */
  jd-loading-button > .jd-button > .jd-loading-button__busy { display: none; }
  jd-loading-button[loading][data-swap] > .jd-button > .jd-loading-button__label { display: none; }
  jd-loading-button[loading][data-swap] > .jd-button > .jd-loading-button__busy { display: inline; }
}`;
