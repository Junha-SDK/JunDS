 ...
 * 현재가 띠 accent 8% 배경 13px.
 * 색 관례: 매도=하락색(--jd-fin-down), 매수=상승색(--jd-fin-up).
 *
 * 등락색 통로: --jd-finance-* → --bm-* → 의미색. 호가창은 잔량 막대·호가 셀 틴트까지
 * 방향색으로 칠하는 면이라, 훅을 비껴가면 한 화면에서 매수/매도 색이 형제 컴포넌트와
 * 정반대로 보인다 — 그래서 배경도 전부 같은 훅에서 color-mix로 뽑는다.
 */
export default css`
