...
export default css`
@layer junds.base {
  jd-nav-sidebar:not(:defined) { display: none; }
}
@layer junds.components {
  ...
  .jd-nav-sidebar__header[hidden] { display: none; }

  .jd-nav-sidebar__nav {
    flex: 1; min-height: 0; overflow-y: auto;
    padding-block: var(--jd-space-4);
  }
  .jd-nav-sidebar__top[hidden] { display: none; }

  .jd-nav-sidebar__section { padding: 0 var(--jd-space-3) var(--jd-space-4); }
  ...
  .jd-nav-sidebar__section-title[hidden] { display: none; }
  ...
  .jd-nav-sidebar__link:hover:not([data-active]) { background: var(--jd-color-border-light); }
  ...
  .jd-nav-sidebar__link[data-active] .jd-nav-sidebar__rail { opacity: 1; }
  ...
  .jd-nav-sidebar__footer[hidden] { display: none; }

  @media (prefers-reduced-motion: reduce) {
    .jd-nav-sidebar__link { transition: none; }
  }
}`;
