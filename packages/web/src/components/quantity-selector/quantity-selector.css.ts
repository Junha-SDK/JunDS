...
@layer junds.components {
  ...
  jd-quantity-selector[error] { border-color: var(--jd-color-danger); }
  jd-quantity-selector[error]:focus-within { box-shadow: var(--jd-shadow-focus-ring-danger); }
  jd-quantity-selector[disabled] { opacity: var(--jd-opacity-50); }

  /* size 기본 md — 버튼 36px 정사각 · 입력 48px 폭 */
  jd-quantity-selector .jd-number-input__step { width: 2.25rem; height: 2.25rem; }
  ...
  jd-quantity-selector[size="sm"] .jd-number-input__step { width: 1.75rem; height: 1.75rem; }
  jd-quantity-selector[size="sm"] .jd-number-input__input {
    width: 2.25rem; height: 1.75rem; font-size: var(--jd-text-xs);
  }
  jd-quantity-selector[size="lg"] .jd-number-input__step { width: 2.75rem; height: 2.75rem; }
  jd-quantity-selector[size="lg"] .jd-number-input__input {
    width: 3.5rem; height: 2.75rem; font-size: var(--jd-text-md);
  }
  ...
  jd-quantity-selector .jd-number-input__input:read-only { cursor: default; }
}`;
