...
@layer junds.components {
  ...
  jd-reading-progress[compact] {
    flex-direction: row; align-items: center; gap: var(--jd-space-2);
  ...
  .jd-reading-progress__cur { font-weight: var(--jd-weight-semibold); color: var(--jd-color-foreground); }
  .jd-reading-progress__sep { margin-inline: var(--jd-space-1); }

  .jd-reading-progress__track {
    position: relative; overflow: hidden;
    height: 0.5rem; border-radius: var(--jd-radius-full);
  ...
  .jd-reading-progress__remain[hidden] { display: none; }

  @media (prefers-reduced-motion: reduce) {
    .jd-reading-progress__fill { transition: none; }
  }
}`;
