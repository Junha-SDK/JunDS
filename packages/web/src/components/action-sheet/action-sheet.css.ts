...
@layer junds.components {
  ...
  .jd-action-sheet__cancel {
    width: 100%; padding: var(--jd-space-3-5) var(--jd-space-4);
    border: 0; background: var(--jd-color-card); cursor: pointer;
  ...
  .jd-action-sheet__cancel:focus-visible { outline: none; box-shadow: var(--jd-shadow-focus-ring); }

  .jd-action-sheet__cancel {
    color: var(--jd-color-foreground); font-weight: var(--jd-weight-semibold);
    border-radius: var(--jd-radius-2xl);
  }

  @media (prefers-reduced-motion: reduce) {
    jd-action-sheet .jd-modal__panel { animation: none; }
  }
}`;
