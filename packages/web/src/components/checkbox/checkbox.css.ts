...
@layer junds.components {
  ...
  jd-checkbox[disabled] > .jd-checkbox { cursor: not-allowed; }
  ...
  .jd-checkbox__input:active:not(:disabled) { scale: 0.92; }
  ...
  .jd-checkbox__input:checked { background-image: var(--_jd-cb-check); }
  /* indeterminate가 checked보다 뒤 — 둘이 동시에 참일 때 대시가 이긴다(OS 관례) */
  .jd-checkbox__input:indeterminate { background-image: var(--_jd-cb-dash); }
  ...
  jd-checkbox[size="sm"] .jd-checkbox__input {
    width: 1rem; height: 1rem; border-radius: var(--jd-radius-sm);
  }
  ...
  jd-checkbox[disabled] .jd-checkbox__label { color: var(--jd-color-muted); }

  @media (prefers-reduced-motion: reduce) {
    .jd-checkbox__input { transition: none; }
    .jd-checkbox__input:active:not(:disabled) { scale: 1; }
  }
}`;
