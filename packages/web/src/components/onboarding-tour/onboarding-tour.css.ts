...
export default css`
@layer junds.base {
  jd-onboarding-tour:not(:defined) { display: none; }
}
@layer junds.components {
  jd-onboarding-tour { display: none; }
  ...
  .jd-onboarding-tour__spot {
    ...
    box-shadow:
      0 0 0 var(--jd-border-heavy) color-mix(in srgb, var(--jd-color-primary) 70%, transparent),
      0 0 0 9999px rgba(0, 0, 0, 0.55);
  }
  .jd-onboarding-tour__spot[hidden] { display: none; }
  ...
  .jd-onboarding-tour__tooltip[hidden] { display: none; }
  ...
  .jd-onboarding-tour__title[hidden] { display: none; }
  ...
  .jd-onboarding-tour__desc[hidden] { display: none; }

  .jd-onboarding-tour__actions {
    display: flex; align-items: center; justify-content: space-between;
    margin-top: var(--jd-space-3); gap: var(--jd-space-3);
  }
  .jd-onboarding-tour__nav { display: flex; align-items: center; gap: var(--jd-space-2); }

  .jd-onboarding-tour__skip,
  .jd-onboarding-tour__prev,
  .jd-onboarding-tour__next { font-family: inherit; cursor: pointer; }
  ...
  .jd-onboarding-tour__skip:hover { color: var(--jd-color-foreground); }

  .jd-onboarding-tour__prev {
    padding: var(--jd-space-1-5) var(--jd-space-3);
    font-size: var(--jd-text-xs); color: var(--jd-color-foreground);
    background: transparent;
    border: var(--jd-border-thin) solid var(--jd-color-border);
    border-radius: var(--jd-radius-md);
    transition: background var(--jd-duration-fast) var(--jd-easing-ease-out);
  }
  .jd-onboarding-tour__prev:hover { background: var(--jd-color-card-hover); }
  .jd-onboarding-tour__prev[hidden] { display: none; }

  .jd-onboarding-tour__next {
    padding: var(--jd-space-1-5) var(--jd-space-3);
    font-size: var(--jd-text-xs); color: #fff;
    background: var(--jd-color-primary); border: 0;
    border-radius: var(--jd-radius-md);
    transition: filter var(--jd-duration-fast) var(--jd-easing-ease-out);
  }
  .jd-onboarding-tour__next:hover { filter: brightness(1.1); }
  ...
  @media (prefers-reduced-motion: no-preference) {
    ...
    .jd-onboarding-tour__spot {
      transition:
        top var(--jd-duration-normal) var(--jd-easing-ease-out),
  ...
  @keyframes jd-onboarding-tour-fade { from { opacity: 0; } }
  @keyframes jd-onboarding-tour-pop { from { opacity: 0; transform: scale(0.96); } }
}`;
