...
@layer junds.components {
  jd-range-slider { display: block; width: 100%; }
  jd-range-slider[disabled] { opacity: var(--jd-opacity-50); }
  ...
  .jd-range-slider__values[hidden] { display: none; }

  .jd-range-slider__track {
    position: relative; height: 1.25rem; display: flex; align-items: center;
    cursor: pointer; touch-action: none;
  }
  jd-range-slider[disabled] .jd-range-slider__track { cursor: not-allowed; }
  ...
  .jd-range-slider__thumb:hover { scale: 1.1; }
  jd-range-slider[data-dragging] .jd-range-slider__thumb { cursor: grabbing; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .jd-range-slider__thumb { transition: none; }
  }
}`;
