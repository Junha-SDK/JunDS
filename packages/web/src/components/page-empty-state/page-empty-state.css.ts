...
@layer junds.components {
  jd-page-empty-state {
    ...
    box-sizing: border-box; text-align: center; font-family: var(--jd-font-sans);
  ...
  jd-page-empty-state[size="sm"] { padding: var(--jd-space-8) var(--jd-space-6); }
  jd-page-empty-state[size="lg"] { padding: var(--jd-space-16) var(--jd-space-6); }

  /* 아이콘 칩 해제 — 이모지/도형을 그대로 크게 */
  jd-page-empty-state .jd-empty-state__icon {
    width: auto; height: auto; background: none;
    margin-block-end: var(--jd-space-4); color: var(--jd-color-muted);
  }
  jd-page-empty-state .jd-page-empty-state__emoji {
    font-size: 40px; line-height: 1; display: block;
  }
  jd-page-empty-state > [slot="action"] { margin-block-start: var(--jd-space-5); }
}`;
