...
export default css`
@layer junds.base {
  jd-notification-center:not(:defined) { display: inline-block; }
  jd-notification-center:not(:defined) > script { display: none; }
}
@layer junds.components {
  /* 호스트 박스 — popover.css는 파생 태그가 각자 선언하도록 요구한다 */
  jd-notification-center { position: relative; display: inline-block; }

  /* 패널 표면 오버라이드 (배치·애니메이션은 공유 시트) */
  jd-notification-center > .jd-popover__panel {
    width: 20rem; max-width: min(20rem, calc(100vw - 2rem));
    padding: 0; overflow: hidden; backdrop-filter: none;
  ...
  .jd-notification-center__bell {
    ...
    border: 0; background: none; color: var(--jd-color-muted);
    border-radius: var(--jd-radius-lg);
    transition: background var(--jd-duration-fast) var(--jd-easing-ease-out);
  }
  .jd-notification-center__bell:hover { background: var(--jd-color-card-hover); }
  ...
  .jd-notification-center__badge {
    ...
    min-width: 18px; height: 18px; padding: 0 var(--jd-space-1);
    background: var(--jd-color-danger); color: #ffffff;
    font-size: 10px; font-weight: var(--jd-weight-semibold);
    font-variant-numeric: tabular-nums; border-radius: var(--jd-radius-full);
  }
  .jd-notification-center__badge[hidden] { display: none; }
  ...
  .jd-notification-center__clear[hidden] { display: none; }
  ...
  /* ── 목록 ── */
  .jd-notification-center__list { max-height: 20rem; overflow-y: auto; }
  .jd-notification-center__empty {
    padding: var(--jd-space-8) var(--jd-space-4); text-align: center;
    font-size: var(--jd-text-sm); color: var(--jd-color-muted);
  }

  .jd-notification-center__item {
    display: flex; gap: var(--jd-space-3); width: 100%; box-sizing: border-box;
    padding: var(--jd-space-3) var(--jd-space-4);
    border: 0; border-block-end: var(--jd-border-thin) solid var(--jd-color-border-light);
    background: none; text-align: start; font-family: inherit; color: inherit;
    transition: background var(--jd-duration-fast) var(--jd-easing-ease-out);
  }
  .jd-notification-center__list > .jd-notification-center__item:last-child {
    border-block-end: 0;
  }
  button.jd-notification-center__item { cursor: pointer; }
  button.jd-notification-center__item:hover { background: var(--jd-color-card-hover); }
  ...
  .jd-notification-center__icon {
    flex-shrink: 0; display: inline-flex; margin-block-start: 2px;
  }
  .jd-notification-center__icon > svg { width: 1.125rem; height: 1.125rem; }

  .jd-notification-center__body { flex: 1 1 auto; min-width: 0; }
  .jd-notification-center__title {
    margin: 0; font-size: var(--jd-text-sm); font-weight: var(--jd-weight-normal);
    color: var(--jd-color-foreground);
  ...
  .jd-notification-center__time {
    margin: 2px 0 0; font-size: 10px; color: var(--jd-color-muted-light);
  }

  .jd-notification-center__dot {
    flex-shrink: 0; width: 8px; height: 8px; margin-block-start: 6px;
