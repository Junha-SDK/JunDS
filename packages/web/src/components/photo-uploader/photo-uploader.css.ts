...
export default css`
@layer junds.base {
  jd-photo-uploader:not(:defined) { display: block; }
}
@layer junds.components {
  ...
  .jd-photo-uploader__grid[hidden] { display: none; }
  @media (min-width: 640px) {
    .jd-photo-uploader__grid { grid-template-columns: repeat(5, minmax(0, 1fr)); }
  }
  ...
  .jd-photo-uploader__remove:hover { background: rgba(0, 0, 0, 0.8); }
  ...
  .jd-photo-uploader__remove > svg { width: 0.75rem; height: 0.75rem; }

  @media (prefers-reduced-motion: reduce) {
    .jd-photo-uploader__zone,
    .jd-photo-uploader__remove { transition: none; }
  }
}`;
