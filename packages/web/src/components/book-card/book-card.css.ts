...
export default css`
@layer junds.base {
  jd-book-card:not(:defined) { display: block; }
}
@layer junds.components {
  jd-book-card { display: block; }
  ...
  .jd-book-card__button:focus-visible {
    outline: none;
    box-shadow:
      0 20px 25px -5px rgba(0,0,0,.2), 0 8px 10px -6px rgba(0,0,0,.2),
      var(--jd-shadow-focus-ring);
  }
  @media (prefers-reduced-motion: no-preference) {
    /* 키보드 사용자도 같은 상승을 받는다(v2는 hover 전용이었다) */
    .jd-book-card__button:hover,
    .jd-book-card__button:focus-visible { transform: translateY(-0.25rem); }
  }
  @media (prefers-reduced-motion: reduce) {
    .jd-book-card__button { transition: none; }
  }
  ...
  .jd-book-card__button:focus-visible .jd-book-card__cover::before { opacity: 1; }
  ...
  .jd-book-card__lock[hidden] { display: none; }
  .jd-book-card__lock-icon { font-size: 1.875rem; line-height: var(--jd-leading-none); }
  ...
  .jd-book-card__kind { color: var(--jd-color-muted); }
  .jd-book-card__kind[hidden] { display: none; }
  .jd-book-card__author { color: var(--jd-color-muted-light); }
  .jd-book-card__author[hidden] { display: none; }

  .jd-book-card__badge { margin-block-start: var(--jd-space-1-5); }
  /* 저작 display가 UA의 [hidden]을 이긴다 — jd-badge는 inline-flex다(레포 관용구) */
  .jd-book-card__badge[hidden] { display: none; }
}`;
