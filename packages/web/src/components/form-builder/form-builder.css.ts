...
@layer junds.components {
  jd-form-builder { display: block; }
  .jd-form-builder { display: flex; flex-direction: column; gap: var(--jd-space-4); }

  .jd-form-builder__grid {
    display: grid; gap: var(--jd-space-4); grid-template-columns: 1fr;
  }
  @media (min-width: 40rem) {
    jd-form-builder[columns="2"] .jd-form-builder__grid { grid-template-columns: 1fr 1fr; }
    jd-form-builder[columns="2"] .jd-form-builder__grid > [data-span] { grid-column: 1 / -1; }
  }
  ...
  .jd-form-builder__input[aria-invalid="true"] { border-color: var(--jd-color-danger); }
  ...
  @media (prefers-reduced-motion: reduce) {
    .jd-form-builder__input { transition: none; }
  }
}`;
