...
const TAG_CONST_RE = /\bconst\s+([A-Za-z_$][\w$]*)\s*=\s*(?:"([^"]+)"|'([^']+)')/g;
...
const banner = "/* AUTO-GENERATED by scripts/gen-exports.mjs — DO NOT EDIT (03-web-arch §6.2) */\n";
const lines = [banner];
...
if (check) {
  ...
  if (drift.length) {
    console.error(
      `✗ gen-exports drift: ${drift.join(", ")} — node scripts/gen-exports.mjs 재실행 필요`,
    );
    process.exit(1);
  }
  console.log(
    `✓ gen-exports drift 없음 (${perDir.length}폴더 · 클래스 ${perDir.reduce(
      (a, b) => a + b.classes.length,
      0,
    )}종)`,
  );
} else {
  ...
  console.log(
    `✓ exports ${Object.keys(exportsMap).length}엔트리 · ALL_COMPONENTS ${perDir.reduce(
      (a, b) => a + b.classes.length,
      0,
    )}종 생성`,
  );
