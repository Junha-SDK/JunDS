...
async function openModal(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    (
      document.querySelector("#m") as HTMLElement & { showModal(): void }
    ).showModal();
  });
  await expect(page.locator("#m .jd-modal__panel")).toBeVisible();
}

test("열기: 패널 표시 + [data-autofocus] 초기 포커스 + body 스크롤 락", async ({
  page,
}) => {
...
test("Tab이 패널 안에서 순환한다 (마지막→첫, Shift+Tab 역방향)", async ({
  page,
  browserName,
}) => {
  await mount(page, MODAL);
  ...
  const closed = page.evaluate(
    () =>
      new Promise((r) =>
        document
          .querySelector("#m")!
          .addEventListener("jd-close", () => r(true), { once: true }),
      ),
  );
...
test("백드롭 클릭으로 닫힌다 — persistent면 무시, ESC는 여전히 동작", async ({
  page,
}) => {
  await mount(page, MODAL);
  await openModal(page);
  ...
  await page
    .locator("#m .jd-modal__backdrop")
    .click({ position: { x: 5, y: 5 } });
  await expect(page.locator("#m .jd-modal__panel")).toBeHidden();

  await page.evaluate(() =>
    document.querySelector("#m")!.setAttribute("persistent", ""),
  );
  await openModal(page);
  await page
    .locator("#m .jd-modal__backdrop")
...
test("jd-request-close preventDefault → 상태 변화 중단 (WEB-04)", async ({
  page,
}) => {
  await mount(page, MODAL);
  await openModal(page);
  await page.evaluate(() =>
    document
      .querySelector("#m")!
      .addEventListener("jd-request-close", (e) => e.preventDefault(), {
        once: true,
      }),
  );
  await page.keyboard.press("Escape");
...
test("중첩 모달은 ESC로 위에서부터 닫히고 마지막까지 스크롤 락을 유지한다", async ({
  page,
}) => {
  await mount(
    page,
  ...
  await page.evaluate(() => {
    (
      document.querySelector("#first") as HTMLElement & { showModal(): void }
    ).showModal();
    (
      document.querySelector("#second") as HTMLElement & { showModal(): void }
    ).showModal();
  });
