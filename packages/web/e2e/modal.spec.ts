...
test("Tab이 패널 안에서 순환한다 (마지막→첫, Shift+Tab 역방향)", async ({ page }) => {
  await mount(page, MODAL);
  await openModal(page);
...
test("ESC: jd-close 발행 + 닫힘 + 오프너로 포커스 복귀", async ({ page }) => {
  ...
  const closed = page.evaluate(
    () => new Promise((r) => document.querySelector("#m")!.addEventListener("jd-close", () => r(true), { once: true })),
  );
...
test("jd-request-close preventDefault → 상태 변화 중단 (WEB-04)", async ({ page }) => {
  ...
  await openModal(page);
  await page.evaluate(() =>
    document.querySelector("#m")!.addEventListener("jd-request-close", (e) => e.preventDefault(), { once: true }),
  );
  await page.keyboard.press("Escape");
