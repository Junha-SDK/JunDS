...
test("jd-text-field error: aria-invalid + aria-describedby가 보이는 메시지를 가리킨다", async ({ page }) => {
...
test("jd-button type=submit이 폼을 제출한다 — disabled/loading은 클릭 불가", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const b = document.querySelector("#go") as HTMLElement & { disabled: boolean };
    b.disabled = true;
  });
  ...
  await page.evaluate(() => {
    const b = document.querySelector("#go") as HTMLElement & { disabled: boolean; loading: boolean };
    b.disabled = false;
    b.loading = true;
  });
  await expect(page.locator("#go button")).toBeDisabled();
...
test("jd-input 실시간 입력 이벤트 — detail.value 정규화 (§1.5)", async ({ page }) => {
  ...
  const seen = page.evaluate(
    () =>
      new Promise((r) =>
        document.querySelector("jd-text-field")!.addEventListener(
          "jd-input",
