...
final class JdWrapViewTests: XCTestCase {
    ...
    private func laidOut(_ wrap: JdWrapView, width: CGFloat) -> JdWrapView {
        wrap.frame = CGRect(x: 0, y: 0, width: width, height: wrap.sizeThatFits(
    ...
    func test_items_flow_on_one_line_when_they_fit() {
        let wrap = JdWrapView(itemSpacing: 10, [FixedBox(40, 20), FixedBox(40, 20), FixedBox(40, 20)])
    ...
    func test_items_wrap_to_next_line_when_they_do_not_fit() {
        let wrap = JdWrapView(itemSpacing: 10, [FixedBox(80, 20), FixedBox(80, 20), FixedBox(80, 20)])
    ...
    func test_reported_height_matches_placed_content() {
        let wrap = JdWrapView(itemSpacing: 10, lineSpacing: 6,
                              [FixedBox(80, 20), FixedBox(80, 30), FixedBox(80, 20)])
        let reported = wrap.sizeThatFits(CGSize(width: 180, height: CGFloat.greatestFiniteMagnitude)).height
        laidOut(wrap, width: 180)
        let placedBottom = wrap.arrangedViews.map(\.frame.maxY).max() ?? 0
        XCTAssertEqual(reported, placedBottom, accuracy: 0.001,
    ...
    func test_line_spacing_defaults_to_item_spacing() {
        ...
        XCTAssertEqual(a.sizeThatFits(CGSize(width: width, height: .infinity)).height,
                       b.sizeThatFits(CGSize(width: width, height: .infinity)).height)
    }

    func test_empty_and_zero_width_are_safe() {
        XCTAssertEqual(JdWrapView(itemSpacing: 8, []).sizeThatFits(CGSize(width: 100, height: 100)).height, 0)
        let wrap = JdWrapView(itemSpacing: 8, [FixedBox(40, 20)])
        XCTAssertEqual(wrap.sizeThatFits(CGSize(width: 0, height: 100)).height, 0,
    ...
    func test_grid_divides_width_equally() {
        let wrap = JdWrapView(itemSpacing: 10, equalWidths: true,
