...
final class JdFeedbackHostTests: XCTestCase {
    ...
    func test_alert_hosts() {
        assertHosts(JdAlert("업데이트 완료", variant: .success, isDismissible: true, onDismiss: {}) {
            Text("세부 내용을 확인하세요")
        })
    }

    func test_banner_hosts() {
        assertHosts(JdBanner("저장되었습니다", variant: .info,
                             actionLabel: "실행 취소", onAction: {},
                             isDismissible: true, onDismiss: {}))
    }

    func test_callout_collapsible_hosts() {
        assertHosts(JdCallout("참고", variant: .tip, isCollapsible: true, initiallyExpanded: true) {
            Text("접을 수 있는 본문")
        })
    }

    func test_callout_static_hosts() {
        assertHosts(JdCallout("경고", variant: .warning) {
            Text("정적 블록 본문")
        })
    }

    func test_notification_hosts() {
        assertHosts(JdNotification(title: "새 알림", description: "설명 텍스트",
    ...
    func test_empty_state_hosts() {
        assertHosts(JdEmptyState(title: "항목 없음", description: "검색어를 바꿔보세요", systemImage: "tray") {
            JdButton("새로 만들기", variant: .primary) {}
        })
    }

    func test_result_hosts() {
        assertHosts(JdResult(status: .success, title: "완료되었습니다", description: "성공적으로 처리했습니다") {
