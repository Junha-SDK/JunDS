...
final class JdChoiceControlHostTests: XCTestCase {
    ...
    func test_jdCheckbox_hosts_and_label_widens() {
        ...
        let labeled = UIHostingController(rootView: JdCheckbox("약관에 동의합니다",
    ...
    func test_jdRadioGroup_hosts_and_axis_changes_shape() {
        ...
        let vertical = UIHostingController(rootView: JdRadioGroup(options, selection: .constant("a")))
            .sizeThatFits(in: CGSize(width: 320, height: 400))
        let horizontal = UIHostingController(rootView: JdRadioGroup(options,
